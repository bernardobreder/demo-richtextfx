...
public abstract class EventSourceHelper<T, E extends Event> {
  ...
  public static <T extends Node, E extends Event> EventSourceHelper<T, E> forNode(
    T node, EventType<? extends E> eventType) {
    return new NodeHelper<>(node, eventType);
  }

  public static <E extends Event> EventSourceHelper<Scene, E> forScene(
    Scene scene, EventType<? extends E> eventType) {
  ...
  abstract <F extends Event> void addEventHandler(T source,
    EventType<F> eventType, EventHandler<? super F> handler);

  abstract <F extends Event> void removeEventHandler(T source,
    EventType<F> eventType, EventHandler<? super F> handler);
...
class NodeHelper<T extends Node, E extends Event> extends
  EventSourceHelper<T, E> {
...
class SceneHelper<E extends Event> extends EventSourceHelper<Scene, E> {
  ...
  @Override
  <F extends Event> void addEventHandler(Scene scene, EventType<F> eventType,
    EventHandler<? super F> handler) {
    scene.addEventHandler(eventType, handler);
  }

  @Override
  <F extends Event> void removeEventHandler(Scene scene, EventType<F> eventType,
    EventHandler<? super F> handler) {
