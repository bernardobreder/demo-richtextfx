...
class InputHandlerTemplateMap<S, E extends Event> {
  ...
  private static <S, E extends Event> Ops<EventType<? extends E>, InputHandlerTemplate<S, ? super E>> ops() {
    return new Ops<EventType<? extends E>, InputHandlerTemplate<S, ? super E>>() {
      @Override
      public boolean isPrefixOf(EventType<? extends E> t1,
        EventType<? extends E> t2) {
        EventType<?> t = t2;
        while (t != null) {
          if (t.equals(t1)) {
            return true;
      ...
      @Override
      public EventType<? extends E> commonPrefix(EventType<? extends E> t1,
        EventType<? extends E> t2) {
      ...
      @Override
      public InputHandlerTemplate<S, ? super E> promote(
        InputHandlerTemplate<S, ? super E> h, EventType<? extends E> subTpe,
        EventType<? extends E> supTpe) {
      ...
      @Override
      public InputHandlerTemplate<S, E> squash(
        InputHandlerTemplate<S, ? super E> v1,
        InputHandlerTemplate<S, ? super E> v2) {
  ...
  private InputHandlerTemplateMap(
    PrefixTree<EventType<? extends E>, InputHandlerTemplate<S, ? super E>> handlerTree) {
    this.handlerTree = handlerTree;
  }

  public <F extends E> void insertAfter(EventType<? extends F> t,
    InputHandlerTemplate<S, ? super F> h) {
    InputHandlerTemplate<S, ? super E> handler =
      (InputHandlerTemplate<S, ? super E>) h;
