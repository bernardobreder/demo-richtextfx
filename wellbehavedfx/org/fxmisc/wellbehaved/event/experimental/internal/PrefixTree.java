...
public abstract class PrefixTree<K, V> {
  ...
  private static class Empty<K, V> extends PrefixTree<K, V> {
    ...
    @Override
    public PrefixTree<K, V> insert(K key, V value,
      BiFunction<? super V, ? super V, ? extends V> combine) {
      return insertInside(key, value, combine);
    }

    @Override
    PrefixTree<K, V> insertInside(K key, V value,
      BiFunction<? super V, ? super V, ? extends V> combine) {
      return new Data<>(ops, key, value);
    }

    @Override
    public <W> PrefixTree<K, W> map(Function<? super V, ? extends W> f,
      Ops<K, W> ops) {
  ...
  private static abstract class NonEmpty<K, V> extends PrefixTree<K, V> {
    ...
    @Override
    public abstract <W> NonEmpty<K, W> map(Function<? super V, ? extends W> f,
      Ops<K, W> ops);

    @Override
    abstract NonEmpty<K, V> insertInside(K key, V value,
      BiFunction<? super V, ? super V, ? extends V> combine);
    ...
    public PrefixTree<K, V> insert(K key, V value,
      ...
      if (ops.isPrefixOf(key, getPrefix())) { // key is a prefix of this tree
        return new Data<>(ops, key, value).insertInside(collapse(), flip(
          combine));
      }
  ...
  private static class Branch<K, V> extends NonEmpty<K, V> {
    ...
    Branch(Ops<K, V> ops, K prefix, List<NonEmpty<K, V>> subTrees) {
      ...
      assert Objects.equals(prefix, subTrees.stream().map(NonEmpty::getPrefix)
        .reduce(ops::commonPrefix).get());
      assert subTrees.stream().noneMatch(tree -> Objects.equals(tree
        .getPrefix(), prefix));
    ...
    private Branch(Ops<K, V> ops, NonEmpty<K, V> t1, NonEmpty<K, V> t2) {
      this(ops, ops.commonPrefix(t1.getPrefix(), t2.getPrefix()), Arrays.asList(
        t1, t2));
    }
    ...
    @Override
    Data<K, V> collapse() {
      return subTrees.stream().map(tree -> tree.collapse().promote(prefix))
        .reduce(Data::squash).get();
    }

    @Override
    NonEmpty<K, V> insertInside(K key, V value,
      BiFunction<? super V, ? super V, ? extends V> combine) {
      ...
      // try to find a sub-tree that has common prefix with key longer than this
      // branch's prefix
      for (int i = 0; i < subTrees.size(); ++i) {
        ...
        if (!Objects.equals(commonPrefix, prefix)) {
          if (Objects.equals(commonPrefix, st.getPrefix())) {
            ...
            return replaceBranch(i, new Data<>(ops, key, value).insertInside(st
              .collapse(), flip(combine)));
          }
          else {
            return replaceBranch(i, new Branch<>(ops, st, new Data<>(ops, key,
              value)));
          }
        }
      }
    ...
    private Branch<K, V> replaceBranch(int i, NonEmpty<K, V> replacement) {
      ...
      assert ops.isPrefixOf(replacement.getPrefix(), subTrees.get(i)
        .getPrefix());
    ...
    @Override
    public <W> NonEmpty<K, W> map(Function<? super V, ? extends W> f,
      Ops<K, W> ops) {
  ...
  private static class Data<K, V> extends NonEmpty<K, V> {
    ...
    NonEmpty<K, V> insertInside(K key, V value,
      ...
      return new Data<>(this.ops, this.key, combine.apply(this.value, ops
        .promote(value, key, this.key)));
    }

    NonEmpty<K, V> insertInside(NonEmpty<K, V> tree,
      BiFunction<? super V, ? super V, ? extends V> combine) {
    ...
    @Override
    public <W> Data<K, W> map(Function<? super V, ? extends W> f,
      Ops<K, W> ops) {
  ...
  public abstract PrefixTree<K, V> insert(K key, V value,
    BiFunction<? super V, ? super V, ? extends V> combine);

  public abstract <W> PrefixTree<K, W> map(Function<? super V, ? extends W> f,
    Ops<K, W> ops);
  ...
  abstract PrefixTree<K, V> insertInside(K key, V value,
    BiFunction<? super V, ? super V, ? extends V> combine);
}
