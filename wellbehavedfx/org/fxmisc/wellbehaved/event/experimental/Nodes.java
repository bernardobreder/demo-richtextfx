...
public class Nodes {
  ...
  private static final String P_INPUTMAP =
    "org.fxmisc.wellbehaved.event.inputmap";

  private static final String P_HANDLERS =
    "org.fxmisc.wellbehaved.event.handlers";
    ...
    if (node.getProperties().get(P_INPUTMAP) == null) {
      ...
      MapChangeListener<Object, Object> listener = ch -> {
        ...
        getHandlers(node).forEach(entry -> {
          node.removeEventHandler((EventType<Event>) entry.getKey(),
        ...
        inputMap.forEachEventType(new HandlerConsumer<Event>() {
          ...
          @Override
          public <E extends Event> void accept(EventType<? extends E> t,
            InputHandler<? super E> h) {
  ...
  private static List<Map.Entry<EventType<?>, EventHandler<?>>> getHandlers(
    Node node) {
    return (List<Entry<EventType<?>, EventHandler<?>>>) node.getProperties()
      .get(P_HANDLERS);
  }
}
