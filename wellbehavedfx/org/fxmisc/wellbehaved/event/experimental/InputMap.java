...
/**
 *
 * @param <E> type of events that this {@linkplain InputMap} <em>may</em>
 *        handle. That is, {@code InputMap<E>} certainly does not handle any
 *        events that are not of type {@code E}; it does <em>not</em> mean it
 *        handles <em>any</em> event of type {@code E}.
 */
@FunctionalInterface
public interface InputMap<E extends Event> {
  ...
  @SafeVarargs
  static <E extends Event> InputMap<E> sequence(
    InputMap<? extends E>... inputMaps) {
  ...
  public static <T extends Event> InputMap<T> consume(
    EventType<? extends T> eventType, Consumer<? super T> action) {
  ...
  public static <T extends Event, U extends T> InputMap<U> consumeWhen(
    EventPattern<? super T, ? extends U> eventPattern,
    BooleanSupplier condition, Consumer<? super U> action) {
    return process(eventPattern, u -> {
  ...
  public static <T extends Event> InputMap<T> consumeWhen(
    EventType<? extends T> eventType, BooleanSupplier condition,
  ...
  public static <T extends Event> InputMap<T> when(BooleanSupplier condition,
    InputMap<T> im) {
    ...
    return new InputMap<T>() {
      ...
      public void forEachEventType(HandlerConsumer<? super T> f) {
        HandlerConsumer<T> g = new HandlerConsumer<T>() {
          ...
          @Override
          public <F extends T> void accept(EventType<? extends F> t,
            InputHandler<? super F> h) {
            f.accept(t, evt -> condition.getAsBoolean() ? h.process(evt)
              : Result.PROCEED);
          }
  ...
  public static <T extends Event> InputMap<T> unless(BooleanSupplier condition,
    InputMap<T> im) {
...
class PatternActionMap<T extends Event, U extends T> implements InputMap<U> {
  ...
  PatternActionMap(EventPattern<T, ? extends U> pattern,
    Function<? super U, InputHandler.Result> action) {
  ...
  public void forEachEventType(HandlerConsumer<? super U> f) {
    InputHandler<T> h = t -> pattern.match(t).map(action::apply).orElse(
      Result.PROCEED);
...
class InputMapChain<E extends Event> implements InputMap<E> {
  ...
  public InputMap<E> without(InputMap<?> that) {
    if (this.equals(that)) {
      ...
      @SuppressWarnings("unchecked")
      InputMap<? extends E>[] ims = (InputMap<? extends E>[]) Stream.of(
        inputMaps).map(im -> im.without(that)).filter(im -> im != EMPTY)
