...
class ListReduction<T> extends ValBase<T> {
  ...
  protected Subscription connect() {
    ...
    return LiveList.observeChanges(input, ch -> {
      for (ListModification<? extends T> mod : ch) {
        ...
        FingerTree<T, T> right = tree.split(mod.getFrom() + mod
          .getRemovedSize())._2;
        FingerTree<T, T> middle = FingerTree.mkTree(mod.getAddedSubList(),
          monoid);
  ...
  protected final T computeValue() {
    if (isObservingInputs()) {
      ...
      assert tree == null;
      int max = input.size();
      return input.subList(getFrom(max), getTo(max)).stream().reduce(reduction)
        .orElse(null);
    }
  }
}
...
class ListRangeReduction<T> extends ListReduction<T> {
  ...
  ListRangeReduction(ObservableList<T> input, ObservableValue<IndexRange> range,
  ...
  @Override
  protected Subscription connect() {
    return super.connect().and(Val.observeInvalidations(range,
      obs -> invalidate()));
  }
