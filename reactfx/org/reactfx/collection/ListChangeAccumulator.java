...
public final class ListChangeAccumulator<E> implements
  ...
  public ListChangeAccumulator<E> add(QuasiListModification<? extends E> mod) {
    if (modifications.isEmpty()) {
      ...
      if (lastOverlapping < firstOverlapping) { // no overlap
        ...
        List<QuasiListModification<? extends E>> overlapping = modifications
          .subList(firstOverlapping, lastOverlapping + 1);
        QuasiListModification<? extends E> joined = join(overlapping, mod
          .getRemoved(), mod.getFrom());
  ...
  public ListChangeAccumulator<E> add(
  ...
  private void offsetPendingModifications(int from, int offset) {
    modifications.subList(from, modifications.size()).replaceAll(
      mod -> new QuasiListModificationImpl<>(mod.getFrom() + offset, mod
        .getRemoved(), mod.getAddedSize()));
  }

  private static <E> QuasiListModification<? extends E> join(
    List<QuasiListModification<? extends E>> mods, List<? extends E> gone,
    int goneOffset) {
    ...
    for (int i = 1; i < mods.size(); ++i) {
      ...
      removedLists.add(gone.subList(prev.getTo() - goneOffset, m.getFrom()
        - goneOffset));
  ...
  private static <E> QuasiListModification<E> combine(
    ...
    if (latter.getFrom() >= former.getFrom() && latter.getFrom() + latter
      .getRemovedSize() <= former.getTo()) {
      // latter is within former
      List<? extends E> removed = former.getRemoved();
      int addedSize = former.getAddedSize() - latter.getRemovedSize() + latter
        .getAddedSize();
      return new QuasiListModificationImpl<>(former.getFrom(), removed,
        addedSize);
    }
    else if (latter.getFrom() <= former.getFrom() && latter.getFrom() + latter
      .getRemovedSize() >= former.getTo()) {
      // former is within latter
      List<E> removed = Lists.concat(latter.getRemoved().subList(0, former
        .getFrom() - latter.getFrom()), former.getRemoved(), latter.getRemoved()
          .subList(former.getTo() - latter.getFrom(), latter.getRemovedSize()));
      return new QuasiListModificationImpl<>(latter.getFrom(), removed, latter
        .getAddedSize());
    }
    else if (latter.getFrom() >= former.getFrom()) {
      // latter overlaps to the right
      List<E> removed = Lists.concat(former.getRemoved(), latter.getRemoved()
        .subList(former.getTo() - latter.getFrom(), latter.getRemovedSize()));
      return new QuasiListModificationImpl<>(former.getFrom(), removed, latter
        .getTo() - former.getFrom());
    }
    else {
      // latter overlaps to the left
      List<E> removed = Lists.concat(latter.getRemoved().subList(0, former
        .getFrom() - latter.getFrom()), former.getRemoved());
      int addedSize = former.getTo() - latter.getRemovedSize() + latter
        .getAddedSize() - latter.getFrom();
      return new QuasiListModificationImpl<>(latter.getFrom(), removed,
        addedSize);
    }
  }
}