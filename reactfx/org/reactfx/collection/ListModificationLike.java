 ...
 * <ul>
 * <li>{@link ListModification} holds a reference to the observable list in
 * which the modification occurred and the
 * {@link ListModification#getAddedSubList()} method returns a sublist view of
 * the original observable list. Such sublist is valid only until the next list
 * modification.
 * <li>{@link MaterializedListModification} has its own copy of added elements (
 * {@link MaterializedListModification#getAdded()}), thus the validity of the
 * list of added elements returned from {@code getAdded()} is not restricted.
 * <li>{@link QuasiListModification} does not provide a way to get the added
 * elements directly, but has to be combined with the observable list to obtain
 * {@linkplain ListModification} (
...
interface ListModificationLike<E> {
  ...
  /**
   * Returns the end position of the change in the modified list. The returned
   * value is equal to {@code getFrom() + getAddedSize()}.
   */
  default int getTo() {
   ...
   * Before the change occurred, the first element of the returned list was at
   * index {@link #getFrom()} in the original list. If no elements were removed
   * by this change, returns an empty list. The size of the returned list is
   * equal to the value returned by {@link #getRemovedSize()}.
   */
