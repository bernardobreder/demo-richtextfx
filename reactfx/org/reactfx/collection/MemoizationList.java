...
class MemoizationListImpl<E> extends LiveListBase<E> implements
  MemoizationList<E>, ReadOnlyLiveListImpl<E> {

  private class MemoizedView extends LiveListBase<E> implements
    ReadOnlyLiveListImpl<E> {
  ...
  protected Subscription observeInputs() {
    ...
    return LiveList.<E> observeQuasiChanges(source, this::sourceChanged).and(
      sparseList::clear);
  }
  ...
  public E get(int index) {
    if (!isObservingInputs()) { // memoization is off
      ...
      if (sparseList.setIfAbsent(index, elem)) {
        memoizedItems.fireElemInsertion(sparseList.getPresentCountBefore(
          index));
      }
      return elem;
    }
    ...
    for (int i = from; i < to; ++i) {
      if (!sparseList.isPresent(i)) {
        ...
        if (sparseList.setIfAbsent(i, elem)) {
          ...
          memoizedItems.prepareNotifications(ProperLiveList.elemInsertion(
            presentBefore));
        }
      }
    }
