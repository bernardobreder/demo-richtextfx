...
public final class Lists {
  ...
  public static void checkPosition(int min, int position, int max) {
    if (!isValidPosition(min, position, max)) {
      throw new IndexOutOfBoundsException(position + " not in [" + min + ", "
        + max + "]");
    }
  }
  ...
  public static void checkRange(int min, int from, int to, int max) {
    if (!isValidRange(min, from, to, max)) {
      throw new IndexOutOfBoundsException("[" + from + ", " + to
        + ") is not a valid range within " + "[" + min + ", " + max + ")");
    }
  }
  ...
  public static boolean isProperNonEmptyRange(int min, int from, int to,
    int max) {
  ...
  public static boolean isStrictlyInsideRange(int min, int from, int to,
    int max) {
    return min < from && from <= to && to < max;
  }

  public static boolean isStrictlyInsideNonEmptyRange(int from, int to,
    int size) {
    return isStrictlyInsideNonEmptyRange(0, from, to, size);
  }

  public static boolean isStrictlyInsideNonEmptyRange(int min, int from, int to,
    int max) {
    return min < from && from < to && to < max;
  }

  public static <E, F> List<F> mappedView(List<? extends E> source,
   ...
   * list is a view of the underlying lists, without copying the elements. The
   * returned list is unmodifiable. Modifications to underlying lists will be
   * visible through the concatenation view.
   */
  ...
  /**
   * Returns a list that is a concatenation of the given lists. The returned
   * list is a view of the underlying lists, without copying the elements. As
   * opposed to {@link #concatView(List)}, the underlying lists must not be
   * modified while the returned concatenation view is in use. On the other
...
class ConcatView<E> extends AbstractList<E> {
  ...
  private static <E> List<E> concatView(List<List<? extends E>> lists,
    ...
    if (len < 1) {
      ...
      int mid = len / 2;
      return new ConcatView<>(concatView(lists.subList(0, mid), false),
...
class ListConcatenation<E> extends AbstractList<E> {
  ...
  static <E> List<E> create(List<List<? extends E>> lists) {
    return lists.stream().filter(l -> !l.isEmpty()).map(l -> {
      ...
      List<E> lst = (List<E>) l;
      return lst instanceof ListConcatenation ? ((ListConcatenation<E>) lst).ft
        : FingerTree.mkTree(Collections.singletonList(lst), LIST_SIZE_MONOID);
    }).reduce(FingerTree::join).<List<E>> map(ListConcatenation<E>::new).orElse(
  ...
  public List<E> subList(int from, int to) {
    Tuple3<FingerTree<List<E>, Integer>, Optional<Tuple2<List<E>, Integer>>, FingerTree<List<E>, Integer>> lmr;
    ...
    t = m.<FingerTree<List<E>, Integer>> map(t2 -> t2.map((lst, i) -> r.prepend(
      lst.subList(i, lst.size())))).orElse(r);

    lmr = t.split(Integer::intValue, to - from);
    l = lmr._1;
    m = lmr._2;

    t = m.<FingerTree<List<E>, Integer>> map(t2 -> t2.map((lst, i) -> l.append(
      lst.subList(0, i)))).orElse(l);
