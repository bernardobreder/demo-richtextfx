...
public interface NotificationAccumulator<O, V, A> {
  ...
  static <T, A> NotificationAccumulator<Consumer<? super T>, T, A> accumulativeStreamNotifications(
    ...
    return new AccumulativeStreamNotifications<>(size, head, tail,
  ...
  static <T> NotificationAccumulator<Consumer<? super T>, T, T> reducingStreamNotifications(
...
abstract class NotificationAccumulatorBase<O, V, A> implements
  NotificationAccumulator<O, V, A>, AccumulationFacility<V, A> {
...
final class NonAccumulativeStreamNotifications<T> extends
  NotificationAccumulatorBase<Consumer<? super T>, T, T> implements
  AccumulationFacility.NoAccumulation<T> {

  @Override
  protected AccumulatorSize size(Consumer<? super T> observer,
    T accumulatedValue) {
...
final class AccumulativeStreamNotifications<T, A> extends
  NotificationAccumulatorBase<Consumer<? super T>, T, A> {
  ...
  AccumulativeStreamNotifications(Function<? super A, AccumulatorSize> size,
    Function<? super A, ? extends T> head,
    Function<? super A, ? extends A> tail,
    Function<? super T, ? extends A> initialTransformation,
    BiFunction<? super A, ? super T, ? extends A> reduction) {
...
final class QueuingStreamNotifications<T> extends
  NotificationAccumulatorBase<Consumer<? super T>, T, Deque<T>> implements
  ...
  @Override
  protected Runnable head(Consumer<? super T> observer,
    Deque<T> accumulatedValue) {
  ...
  @Override
  protected Deque<T> tail(Consumer<? super T> observer,
    Deque<T> accumulatedValue) {
...
abstract class AbstractReducingStreamNotifications<T> extends
  NotificationAccumulatorBase<Consumer<? super T>, T, T> implements
  AccumulationFacility.HomotypicAccumulation<T> {

  @Override
  protected final AccumulatorSize size(Consumer<? super T> observer,
    T accumulatedValue) {
    return AccumulatorSize.ONE;
  }

  @Override
  protected final Runnable head(Consumer<? super T> observer,
    T accumulatedValue) {
    return () -> observer.accept(accumulatedValue);
  }
...
final class ReducingStreamNotifications<T> extends
...
final class RetainLatestStreamNotifications<T> extends
  AbstractReducingStreamNotifications<T> implements
...
final class RetailOldestValNotifications<T> extends
  AbstractReducingStreamNotifications<T> implements
...
final class ListNotifications<E> extends
  NotificationAccumulatorBase<LiveList.Observer<? super E, ?>, QuasiListChange<? extends E>, ListModificationSequence<E>>
  implements AccumulationFacility.ListChangeAccumulation<E> {

  @Override
  protected AccumulatorSize size(Observer<? super E, ?> observer,
    ListModificationSequence<E> accumulatedValue) {
    return observer.sizeOf(accumulatedValue);
  }

  @Override
  protected Runnable head(Observer<? super E, ?> observer,
    ListModificationSequence<E> mods) {
    return takeHead(observer, mods);
  }

  private final <O> Runnable takeHead(Observer<? super E, O> observer,
    ListModificationSequence<E> mods) {
  ...
  @Override
  protected ListModificationSequence<E> tail(Observer<? super E, ?> observer,
    ListModificationSequence<E> mods) {
