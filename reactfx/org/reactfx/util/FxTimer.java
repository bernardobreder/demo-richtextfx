...
public class FxTimer implements Timer {
  ...
  /**
   * Prepares a (stopped) timer that lasts for {@code delay} and whose action
   * runs when timer <em>ends</em>.
   */
  ...
  /**
   * Prepares a (stopped) timer that lasts for {@code interval} and that
   * executes the given action periodically when the timer <em>ends</em>.
   */
  public static Timer createPeriodic(java.time.Duration interval,
    Runnable action) {
   ...
   */
  public static Timer runPeriodically(java.time.Duration interval,
    Runnable action) {
  ...
  /**
   * Prepares a (stopped) timer that lasts for {@code interval} and that
   * executes the given action periodically when the timer <em>starts</em>.
   */
  public static Timer createPeriodic0(java.time.Duration interval,
    Runnable action) {
    return new FxTimer(java.time.Duration.ZERO, interval, action,
   ...
   */
  public static Timer runPeriodically0(java.time.Duration interval,
    Runnable action) {
  ...
  private FxTimer(java.time.Duration actionTime, java.time.Duration period,
    Runnable action, int cycles) {
    this.actionTime = Duration.millis(actionTime.toMillis());
    this.timeline = new Timeline();
    this.action = action;

    timeline.getKeyFrames().add(new KeyFrame(this.actionTime)); // used as
                                                                // placeholder
