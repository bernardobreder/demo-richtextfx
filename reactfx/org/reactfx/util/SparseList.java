...
public final class SparseList<E> {
  ...
  private static final MapToMonoid<Segment<?>, Stats> SEGMENT_STATS =
    new MapToMonoid<Segment<?>, Stats>() {
      ...
      public Stats reduce(Stats left, Stats right) {
        return new Stats(left.size + right.size, left.presentCount
          + right.presentCount);
      }
  ...
  public int getPresentCountBefore(int position) {
    ...
    return tree.getStatsBetween(Stats::getSize, 0, position,
  ...
  public int indexOfPresentItem(int presentIndex) {
    ...
    return tree.locateProgressively(Stats::getPresentCount, presentIndex).map(
      this::locationToPosition);
  }
  ...
  public IndexRange getPresentItemsRange() {
    if (getPresentCount() == 0) {
      ...
      int lowerBound = tree.locateProgressively(Stats::getPresentCount, 0).map(
        this::locationToPosition);
      int upperBound = tree.locateRegressively(Stats::getPresentCount,
        getPresentCount()).map(this::locationToPosition);
  ...
  public List<E> collect(int from, int to) {
    ...
    return tree.foldBetween(acc, (l, seg) -> seg.appendTo(l), Stats::getSize,
      from, to, (l, seg, start, end) -> seg.appendRangeTo(l, start, end));
  }
  ...
  public void splice(int from, int to, Collection<? extends E> elems) {
    if (elems.isEmpty()) {
      ...
      spliceSegments(from, to, Collections.singletonList(new PresentSegment<>(
        elems)));
    }
  }
  ...
  public void spliceByVoid(int from, int to, int length) {
    if (length == 0) {
      ...
      spliceSegments(from, to, Collections.singletonList(new AbsentSegment<>(
        length)));
    }
  }
  ...
  private FingerTree<Segment<E>, Stats> join(FingerTree<Segment<E>, Stats> left,
    Optional<Tuple2<Segment<E>, Integer>> lSuffix, Segment<E> middle,
    Optional<Tuple2<Segment<E>, Integer>> rPrefix,
    FingerTree<Segment<E>, Stats> right) {
    return join(left, lSuffix, Collections.singletonList(middle), rPrefix,
      right);
  }

  private FingerTree<Segment<E>, Stats> join(FingerTree<Segment<E>, Stats> left,
    Optional<Tuple2<Segment<E>, Integer>> lSuffix, List<Segment<E>> middle,
    Optional<Tuple2<Segment<E>, Integer>> rPrefix,
    FingerTree<Segment<E>, Stats> right) {
  ...
  private FingerTree<Segment<E>, Stats> join(FingerTree<Segment<E>, Stats> left,
    List<Segment<E>> middle, FingerTree<Segment<E>, Stats> right) {
  ...
  private FingerTree<Segment<E>, Stats> append(
    FingerTree<Segment<E>, Stats> left, Segment<E> right) {
    if (left.isEmpty()) {
