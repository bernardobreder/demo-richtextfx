...
public abstract class FingerTree<T, S> {
  ...
  private static final class Empty<T, S> extends FingerTree<T, S> {
    ...
    @Override
    BiIndex locateProgressively0(ToIntFunction<? super S> metric,
      int position) {
      throw new IndexOutOfBoundsException();
    }
    ...
    <R> R foldBetween0(R acc,
      BiFunction<? super R, ? super T, ? extends R> reduction, int startLeaf,
      int endLeaf) {
    ...
    <R> R foldBetween0(R acc,
      ...
      ToIntFunction<? super S> metric, int startPosition, int endPosition,
    ...
    @Override
    S getStatsBetween0(ToIntFunction<? super S> metric, int startPosition,
      int endPosition,
  ...
  private static class Leaf<T, S> extends FingerTree<T, S> {
    ...
    @Override
    S getStatsBetween0(int startLeaf, int endLeaf) {
      assert Lists.isNonEmptyRange(startLeaf, endLeaf,
        getLeafCount()) : "Didn't expect empty range [" + startLeaf + ", "
          + endLeaf + ")";
      return getStats();
    }
    ...
    S getStatsBetween0(ToIntFunction<? super S> metric, int startPosition,
      ...
      assert Lists.isNonEmptyRange(startPosition, endPosition, measure(
        metric)) : "Didn't expect empty range [" + startPosition + ", "
          + endPosition + ")";
  ...
  private static final class Branch<T, S> extends FingerTree<T, S> {
    ...
    private Branch(MapToMonoid<? super T, S> monoid,
      ...
      this.stats = children.mapReduce1(FingerTree<T, S>::getStats,
        monoid::reduce);
    }
    ...
    private Cons<FingerTree<T, S>> updateLeaf0(int index, T data,
    ...
    @Override
    final BiIndex locateProgressively0(ToIntFunction<? super S> metric,
      int position) {
      assert Lists.isValidPosition(position, measure(metric));
      return locateProgressively0(metric, position, children);
    }

    private BiIndex locateProgressively0(ToIntFunction<? super S> metric,
      int position, LL<? extends FingerTree<T, S>> nodes) {
      FingerTree<T, S> head = nodes.head();
      int headLen = head.measure(metric);
      if (position < headLen || (position == headLen && nodes.tail()
        .isEmpty())) {
    ...
    @Override
    final BiIndex locateRegressively0(ToIntFunction<? super S> metric,
      int position) {
      assert Lists.isValidPosition(position, measure(metric));
      return locateRegressively0(metric, position, children);
    }

    private BiIndex locateRegressively0(ToIntFunction<? super S> metric,
      int position, LL<? extends FingerTree<T, S>> nodes) {
      FingerTree<T, S> head = nodes.head();
      int headLen = head.measure(metric);
      if (position <= headLen) {
    ...
    @Override
    final <R> R fold(R acc,
      BiFunction<? super R, ? super T, ? extends R> reduction) {
      return children.fold(acc, (r, n) -> n.fold(r, reduction));
    }

    @Override
    final <R> R foldBetween0(R acc,
      BiFunction<? super R, ? super T, ? extends R> reduction, int startLeaf,
      int endLeaf) {
    ...
    private <R> R foldBetween0(R acc,
      BiFunction<? super R, ? super T, ? extends R> reduction, int startLeaf,
      int endLeaf, LL<? extends FingerTree<T, S>> nodes) {
      FingerTree<T, S> head = nodes.head();
      int headSize = head.getLeafCount();
    ...
    final <R> R foldBetween0(R acc,
      ...
      return foldBetween0(acc, reduction, metric, startPosition, endPosition,
        rangeReduction, children);
    }
    ...
    private <R> R foldBetween0(R acc,
      ...
      if (startPosition < headTo) {
        acc = head.foldBetween0(acc, reduction, metric, startPosition, headTo,
          rangeReduction);
      }
      if (tailFrom < tailTo) {
        acc = foldBetween0(acc, reduction, metric, tailFrom, tailTo,
          rangeReduction, nodes.tail());
      }
      return acc;
    }
    ...
    private S getStatsBetween0(int startLeaf, int endLeaf,
      LL<? extends FingerTree<T, S>> nodes) {
      FingerTree<T, S> head = nodes.head();
      int headSize = head.getLeafCount();
      int headTo = Math.min(endLeaf, headSize);
      ...
      if (startLeaf < headTo && tailFrom < tailTo) {
        return monoid.reduce(head.getStatsBetween0(startLeaf, headTo),
        ...
        throw new AssertionError("Didn't expect empty range: " + "[" + startLeaf
          + ", " + endLeaf + ")");
      }
    }

    @Override
    final S getStatsBetween0(ToIntFunction<? super S> metric, int startPosition,
      int endPosition,
      TriFunction<? super T, Integer, Integer, ? extends S> subStats) {
    ...
    private S getStatsBetween0(ToIntFunction<? super S> metric,
      ...
      if (startPosition < headTo && tailFrom < tailTo) {
        return monoid.reduce(head.getStatsBetween0(metric, startPosition,
          headTo, subStats), getStatsBetween0(metric, tailFrom, tailTo,
            subStats, nodes.tail()));
      }
        ...
        return getStatsBetween0(metric, tailFrom, tailTo, subStats, nodes
          .tail());
      }
      else {
        throw new AssertionError("Didn't expect empty range: [" + startPosition
          + ", " + endPosition + ")");
      }
    }
    ...
    Either<FingerTree<T, S>, Tuple2<FingerTree<T, S>, FingerTree<T, S>>> appendLte(
      ...
      if (suffix.getDepth() == this.getDepth()) {
        ...
        return children.mapFirst2((left, right) -> {
          return right.appendLte(suffix).unify(r -> left(branch(left, r)),
        ...
        return children.mapFirst3((left, middle, right) -> {
          return right.appendLte(suffix)
            .<Either<FingerTree<T, S>, Tuple2<FingerTree<T, S>, FingerTree<T, S>>>> unify(
              r -> left(branch(left, middle, r)), mr -> right(t(branch(left,
                middle), mr.map(this::branch))));
        });
      }
    }
    ...
    Either<FingerTree<T, S>, Tuple2<FingerTree<T, S>, FingerTree<T, S>>> prependLte(
      ...
      if (prefix.getDepth() == this.getDepth()) {
        ...
        return children.mapFirst2((left, right) -> {
          return left.prependLte(prefix).unify(l -> left(branch(l, right)),
        ...
        return children.mapFirst3((left, middle, right) -> {
          return left.prependLte(prefix)
            .<Either<FingerTree<T, S>, Tuple2<FingerTree<T, S>, FingerTree<T, S>>>> unify(
              l -> left(branch(l, middle, right)), lm -> right(t(lm.map(
                this::branch), branch(middle, right))));
        });
      }
    }
    ...
    private Tuple2<FingerTree<T, S>, FingerTree<T, S>> split0(int beforeLeaf,
      LL<? extends FingerTree<T, S>> nodes) {
      assert beforeLeaf > 0;
      FingerTree<T, S> head = nodes.head();
      int headSize = head.getLeafCount();
      if (beforeLeaf <= headSize) {
        return head.split0(beforeLeaf).map((l, r) -> t(l, concat(cons(r, nodes
          .tail()))));
      }
      else {
        return split0(beforeLeaf - headSize, nodes.tail()).map((l, r) -> t(head
          .appendTree(l), r));
      }
    }
    ...
    private Tuple3<FingerTree<T, S>, Optional<Tuple2<T, Integer>>, FingerTree<T, S>> split0(
      ...
      int headLen = head.measure(metric);
      if (position <= headLen) {
        return head.split0(metric, position).map((l, m, r) -> t(l, m, concat(
          cons(r, nodes.tail()))));
      }
      else {
        return split0(metric, position - headLen, nodes.tail()).map((l, m,
          r) -> t(head.appendTree(l), m, r));
      }
    }
  }
  ...
  public static <T, S> FingerTree<T, S> mkTree(List<? extends T> initialItems,
    MapToMonoid<? super T, S> statisticsProvider) {
  ...
  private static <T, S> FingerTree<T, S> concat(
    ...
    FingerTree<T, S> head = nodes.head();
    return nodes.tail().fold(head, (v, w) -> v.appendTree(w));
  }
  ...
  public <E> E get(ToIntFunction<? super S> metric, int index,
    BiFunction<? super T, Integer, ? extends E> leafAccessor) {
    return locateProgressively(metric, index).map((major, minor) -> leafAccessor
      .apply(getLeaf(major), minor));
  }
  ...
  abstract BiIndex locateProgressively0(ToIntFunction<? super S> metric,
    int position);
  ...
  abstract BiIndex locateRegressively0(ToIntFunction<? super S> metric,
    int position);

  abstract <R> R fold(R acc,
    BiFunction<? super R, ? super T, ? extends R> reduction);
  ...
  abstract S getStatsBetween0(int startLeaf, int endLeaf);

  public S getStatsBetween(ToIntFunction<? super S> metric, int startPosition,
    int endPosition,
    TriFunction<? super T, Integer, Integer, ? extends S> subStats) {
    Lists.checkRange(startPosition, endPosition, measure(metric));
    if (startPosition == endPosition) {
  ...
  abstract S getStatsBetween0(ToIntFunction<? super S> metric,
    int startPosition, int endPosition,
    TriFunction<? super T, Integer, Integer, ? extends S> subStats);
  ...
  public FingerTree<T, S> insertLeaf(int position, T data) {
    ...
    return split0(position).map((l, r) -> l.appendTree(leaf(data)).appendTree(
      r));
  }
  ...
  abstract Either<FingerTree<T, S>, Tuple2<FingerTree<T, S>, FingerTree<T, S>>> appendLte(
    FingerTree<T, S> right);

  abstract Either<FingerTree<T, S>, Tuple2<FingerTree<T, S>, FingerTree<T, S>>> prependLte(
    FingerTree<T, S> left);
  ...
  Branch<T, S> branch(FingerTree<T, S> left, FingerTree<T, S> middle,
    FingerTree<T, S> right) {
