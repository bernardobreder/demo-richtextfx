...
public interface SuspendableEventStream<T> extends EventStream<T>, Suspendable {
  ...
  /**
   * Returns an event stream that is suspended when the given {@code condition}
   * is {@code true} and emits normally when {@code condition} is {@code false}.
   */
...
abstract class SuspendableEventStreamBase<T, A> extends
  SuspendableBase<Consumer<? super T>, T, A> implements ProperEventStream<T>,
...
final class AccumulativeEventStream<T, A> extends
  ...
  AccumulativeEventStream(EventStream<T> source,
    ...
    super(source, NotificationAccumulator.accumulativeStreamNotifications(size,
      head, tail, initialTransformation, accumulation));
    this.size = size;
    this.head = head;
    this.tail = tail;
  }
 ...
 */
final class PausableEventStream<T> extends
  SuspendableEventStreamBase<T, Deque<T>> {
...
abstract class AbstractReducibleEventStream<T> extends
  SuspendableEventStreamBase<T, T> {

  protected AbstractReducibleEventStream(EventStream<T> source,
...
final class ReducibleEventStream<T> extends AbstractReducibleEventStream<T> {
  ...
  public ReducibleEventStream(EventStream<T> source,
    ...
    super(source, NotificationAccumulator.reducingStreamNotifications(
      reduction));
  }
}
...
final class SuppressibleEventStream<T> extends
  SuspendableEventStreamBase<T, T> {
