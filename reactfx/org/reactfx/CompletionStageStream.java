...
public interface CompletionStageStream<T> extends
  EventStream<CompletionStage<T>> {
   ...
   *
   * <p>
   * Note that results from the returned stream may arrive in different order
   * than the completion stages emitted from this stream, due to asynchrony.
   *
   * <p>
   * If a completion stage emitted by this stream fails with exception {@code e}
   ...
   * application thread.
   *
   * @param clientThreadExecutor single-thread executor that executes actions on
   *        the same thread on which this event stream lives.
   * @see #await()
   */
  ...
  /**
   * Similar to {@link #awaitLatest()}, with one addition: When an event is
   * emitted from {@code canceller}, if the completion stage most recently
   * emitted from this stream has not yet completed, its result is discarded
   * (i.e. not emitted from the returned stream).
   *
   ...
   *        completion stage will arrive from this stream shortly, which makes
   *        the currently expected result outdated.
   * @see #awaitLatest()
   */
  ...
  /**
   * A variant of {@link #awaitLatest(EventStream)} for streams that do not live
  ...
  default AwaitingEventStream<Try<T>> awaitLatest(EventStream<?> canceller,
    Executor clientThreadExecutor) {
    return AwaitLatest.awaitCompletionStage(this, canceller,
