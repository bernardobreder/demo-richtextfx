 ...
 */
public abstract class ProxyObservable<O, P, U extends Observable<P>> implements
  Observable<O> {
   ...
   *
   * <p>
   * <strong>Important:</strong> It is required that the transformation applied
   * to two observers that are _equal_ yields two adapted observers that are
   * _equal_. In other words, if `o1.equals(o2)`, then it must be the case that
  ...
  public final void addObserver(O observer) {
    ...
    assert adapted.equals(adaptObserver(
      observer)) : "Two adaptations of the same observer resulted in non-equal"
        + " adapted observers";
