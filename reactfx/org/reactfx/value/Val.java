 ...
 *
 * <p>
 * Canonical observer of {@code Val<T>} is an <em>invalidation observer</em> of
 * type {@code Consumer<? super T>}, which accepts the _invalidated_ value. This
 * is different from {@linkplain InvalidationListener}, which does not accept
...
public interface Val<T> extends ObservableValue<T>,
  ...
  default void removeListener(InvalidationListener listener) {
    removeInvalidationObserver(new InvalidationListenerWrapper<>(this,
      listener));
  }
  ...
  /**
   * Adds a change listener and returns a Subscription that can be used to
  ...
  /**
   * Returns a stream of changed values, which emits the changed value (i.e. the
   * old and the new value) on each change of this observable value.
   */
   ...
   *
   * @param defaultValue value to return if there is no value present in this
   *        {@linkplain Val}.
   */
  ...
  /**
   * Like {@link #getOrElse(Object)}, except the default value is computed by
   * {@code defaultSupplier} only when necessary.
   *
   ...
   * {@linkplain Val}, or and empty {@code Optional} if this {@linkplain Val} is
   * empty.
   */
  ...
  /**
   * Returns a new {@linkplain Val} that holds the same value as this
   * {@linkplain Val} when the value satisfies the predicate and is empty when
   * this {@linkplain Val} is empty or its value does not satisfy the given
   * predicate.
   */
  ...
  /**
   * Like {@link #map(Function)}, but also allows dynamically changing map
   * function.
   */
  ...
  /**
   * Returns a new {@linkplain Val} that, when this {@linkplain Val} holds value
   * {@code x}, holds the value held by {@code f(x)}, and is empty when this
   * {@linkplain Val} is empty.
   */
  ...
  /**
   * Similar to {@link #flatMap(Function)}, except the returned Val is also a
   * Var. This means you can call {@code setValue()} and {@code bind()} methods
   * on the returned value, which delegate to the currently selected Property.
   *
   * <p>
   * As the value of this {@linkplain Val} changes, so does the selected
   * Property. When the Var returned from this method is bound, as the selected
   * Property changes, the previously selected Property is unbound and the newly
   * selected Property is bound.
   *
   * <p>
   * Note that if the currently selected Property is {@code null}, then calling
   * {@code getValue()} on the returned value will return {@code null}
   ...
   * {@linkplain Val} observes {@code condition} whenever it itself has at least
   * one observer and observes {@code this} {@linkplain Val} whenever it itself
   * has at least one observer <em>and</em> the value of {@code condition} is
   * {@code true}. When {@code condition} is {@code true}, the returned
   * {@linkplain Val} has the same value as this {@linkplain Val}. When
   * {@code condition} is {@code false}, the returned {@linkplain Val} has the
   * value that was held by this {@linkplain Val} at the time when
   * {@code condition} changed to {@code false}.
   */
  ...
  /**
   * Returns a new {@linkplain Val} that gradually transitions to the value of
   * this {@linkplain Val} every time this {@linkplain Val} changes.
   *
   * <p>
   * When the returned {@linkplain Val} has no observer, there is no gradual
   * transition taking place. This means that there is no animation running in
   * the background that would consume system resources. This also means that in
   * that case {@link #getValue()} always returns the target value (i.e. the
   * current value of this {@linkplain Val}), instead of any intermediate
   * interpolated value.
   *
   ...
   *        values, given a fraction.
   */
  default Val<T> animate(BiFunction<? super T, ? super T, Duration> duration,
  ...
  /**
   * Let's this {@linkplain Val} be viewed as a {@linkplain Var}, with the given
   * {@code setValue} function serving the purpose of
  ...
  /**
   * Returns a {@linkplain LiveList} view of this {@linkplain Val}. The returned
   * list will have size 1 when this {@linkplain Val} is present (i.e. not
   * {@code null}) and size 0 otherwise.
   */
  ...
  /**
   * Returns a {@linkplain Val} wrapper around {@linkplain ObservableValue}. If
   * the argument is already a {@code Val<T>}, no wrapping occurs and the
   * argument is returned as is. Note that one rarely needs to use this method,
   * because most of the time one can use the appropriate static method directly
   * to get the desired result. For example, instead of
   *
   * <pre>
   * {@code
   ...
   * {@link #observeInvalidations(Consumer)}, or {@link #invalidations()} is
   * needed, since there is no direct static method equivalent for them.
   */
  static <T> Val<T> wrap(ObservableValue<T> obs) {
    return obs instanceof Val ? (Val<T>) obs : new ValWrapper<>(obs);
  }

  static <T> Subscription observeChanges(ObservableValue<? extends T> obs,
  ...
  static Subscription observeInvalidations(ObservableValue<?> obs,
  ...
  static <T> Val<T> orElse(ObservableValue<? extends T> src,
  ...
  static <T, U> Val<U> map(ObservableValue<T> src,
    Function<? super T, ? extends U> f) {
    return new MappedVal<>(src, f);
  }

  static <T, U> Val<U> mapDynamic(ObservableValue<T> src,
    ObservableValue<? extends Function<? super T, ? extends U>> f) {
    return combine(src, f, (t, fn) -> t == null || fn == null ? null
      : fn.apply(t));
  }

  static <T, U> Val<U> flatMap(ObservableValue<T> src,
    Function<? super T, ? extends ObservableValue<U>> f) {
    return new FlatMappedVal<>(src, f);
  }

  static <T, U> Var<U> selectVar(ObservableValue<T> src,
    Function<? super T, ? extends Property<U>> f) {
    return new FlatMappedVar<>(src, f);
  }

  static <T, U> Var<U> selectVar(ObservableValue<T> src,
    Function<? super T, ? extends Property<U>> f, U resetToOnUnbind) {
    return new FlatMappedVar<>(src, f, resetToOnUnbind);
  }

  static <T> Val<T> conditionOn(ObservableValue<T> obs,
  ...
  /**
   * Creates a new {@linkplain Val} that gradually transitions to the value of
   * the given {@linkplain ObservableValue} {@code obs} every time {@code obs}
   * changes.
   *
   * <p>
   * When the returned {@linkplain Val} has no observer, there is no gradual
   ...
   * the background that would consume system resources. This also means that in
   * that case {@link #getValue()} always returns the target value (i.e. the
   * current value of {@code obs}), instead of any intermediate interpolated
   * value.
   *
   ...
   *        values, given a fraction.
   */
  static <T> Val<T> animate(ObservableValue<T> obs,
   ...
   *        values, given a fraction.
   */
  static <T> Val<T> animate(ObservableValue<T> obs, Duration duration,
    Interpolator<T> interpolator) {
    return animate(obs, (a, b) -> duration, interpolator);
  }

  /**
   * Like {@link #animate(ObservableValue, BiFunction, Interpolator)}, but uses
   * the interpolation defined by the {@linkplain Interpolatable} type {@code T}
   * .
   */
  static <T extends Interpolatable<T>> Val<T> animate(ObservableValue<T> obs,
    BiFunction<? super T, ? super T, Duration> duration) {
    return animate(obs, duration, Interpolator.get());
  }

  /**
   * Like {@link #animate(ObservableValue, Duration, Interpolator)}, but uses
   * the interpolation defined by the {@linkplain Interpolatable} type {@code T}
   * .
   */
  static <T extends Interpolatable<T>> Val<T> animate(ObservableValue<T> obs,
  ...
  static <A, B, R> Val<R> combine(ObservableValue<A> src1,
    ObservableValue<B> src2, BiFunction<? super A, ? super B, ? extends R> f) {
    return create(() -> {
  ...
  static <A, B, C, R> Val<R> combine(ObservableValue<A> src1,
    ObservableValue<B> src2, ObservableValue<C> src3,
    TriFunction<? super A, ? super B, ? super C, ? extends R> f) {
    return create(() -> {
      if (src1.getValue() != null && src2.getValue() != null && src3
        .getValue() != null) {
  ...
  static <A, B, C, D, R> Val<R> combine(ObservableValue<A> src1,
    ObservableValue<B> src2, ObservableValue<C> src3, ObservableValue<D> src4,
    TetraFunction<? super A, ? super B, ? super C, ? super D, ? extends R> f) {
    return create(() -> {
      if (src1.getValue() != null && src2.getValue() != null && src3
        .getValue() != null && src4.getValue() != null) {
        return f.apply(src1.getValue(), src2.getValue(), src3.getValue(), src4
          .getValue());
      }
      else {
        return null;
  ...
  static <A, B, C, D, E, R> Val<R> combine(ObservableValue<A> src1,
    ObservableValue<B> src2, ObservableValue<C> src3, ObservableValue<D> src4,
    ObservableValue<E> src5,
    PentaFunction<? super A, ? super B, ? super C, ? super D, ? super E, ? extends R> f) {
    return create(() -> {
      if (src1.getValue() != null && src2.getValue() != null && src3
        .getValue() != null && src4.getValue() != null && src5
          .getValue() != null) {
        return f.apply(src1.getValue(), src2.getValue(), src3.getValue(), src4
          .getValue(), src5.getValue());
      }
      else {
        return null;
      }
  ...
  static <A, B, C, D, E, F, R> Val<R> combine(ObservableValue<A> src1,
    ObservableValue<B> src2, ObservableValue<C> src3, ObservableValue<D> src4,
    ObservableValue<E> src5, ObservableValue<F> src6,
    HexaFunction<? super A, ? super B, ? super C, ? super D, ? super E, ? super F, ? extends R> f) {
    return create(() -> {
      if (src1.getValue() != null && src2.getValue() != null && src3
        .getValue() != null && src4.getValue() != null && src5
          .getValue() != null && src6.getValue() != null) {
        return f.apply(src1.getValue(), src2.getValue(), src3.getValue(), src4
          .getValue(), src5.getValue(), src6.getValue());
      }
      else {
        return null;
      }
  ...
  static <T> Val<T> create(Supplier<? extends T> computeValue,
  ...
  /**
   * Returns a constant {@linkplain Val} that holds the given value. The value
   * never changes and no notifications are ever produced.
   */
  ...
  /**
   * Returns a {@linkplain Val} whose value is {@code true} when {@code node} is
   * <em>showing</em>: it is part of a scene graph ({@link Node#sceneProperty()}
   * is not {@code null}), its scene is part of a window (
   * {@link Scene#windowProperty()} is not {@code null}) and the window is
   * showing ({@link Window#showingProperty()} is {@code true}).
   */
  static Val<Boolean> showingProperty(Node node) {
    return Val.flatMap(node.sceneProperty(), Scene::windowProperty).flatMap(
      Window::showingProperty).orElseConst(false);
  }
}

class InvalidationListenerWrapper<T> extends WrapperBase<InvalidationListener>
  implements Consumer<T> {
  private final ObservableValue<T> obs;

  public InvalidationListenerWrapper(ObservableValue<T> obs,
    InvalidationListener listener) {
    super(listener);
    this.obs = obs;
  }
...
class ChangeListenerWrapper<T> extends WrapperBase<ChangeListener<? super T>>
  implements Consumer<T> {
  private final ObservableValue<T> obs;

  public ChangeListenerWrapper(ObservableValue<T> obs,
    ChangeListener<? super T> listener) {
    super(listener);
    this.obs = obs;
  }
