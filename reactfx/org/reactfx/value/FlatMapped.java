...
abstract class FlatMapped<T, U, O extends ObservableValue<U>> extends
  ValBase<U> {
  final Val<O> src;

  private Subscription selectedSubscription = null; // irrelevant when not
                                                    // connected
  ...
  protected final Subscription connect() {
    return Val.observeInvalidations(src, obs -> srcInvalidated()).and(
  ...
  private void startObservingSelected() {
    ...
    if (selectedSubscription == null) {
      src.ifPresent(sel -> {
        selectedSubscription = Val.observeInvalidations(sel,
          obs -> selectedInvalidated());
      });
    }
  }
...
class FlatMappedVal<T, U, O extends ObservableValue<U>> extends
  FlatMapped<T, U, O> {
...
class FlatMappedVar<T, U, O extends Property<U>> extends FlatMapped<T, U, O>
  implements Var<U> {
  ...
  private FlatMappedVar(ObservableValue<T> src, Function<? super T, O> f,
    Consumer<O> onUnbind) {
    super(src, f);
