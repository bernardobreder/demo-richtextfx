...
class SuspendableValWrapper<T> extends
  SuspendableBase<Consumer<? super T>, T, T> implements SuspendableVal<T>,
  ProperVal<T> {
  private final Val<T> delegate;

  protected SuspendableValWrapper(Val<T> obs) {
    super(obs.invalidations(), NotificationAccumulator
  ...
  protected T tailOf(T accum) {
    throw new UnsupportedOperationException(
      "Cannot take a tail of a single value");
  }
