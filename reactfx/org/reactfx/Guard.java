...
public interface Guard extends AutoCloseable {
  ...
  /**
   * Returns a guard that is a composition of multiple guards. Its
   * {@code close()} method closes the guards in reverse order.
   *
   * @param guards guards that should be released (in reverse order) when the
   *        returned guards is released.
   */
