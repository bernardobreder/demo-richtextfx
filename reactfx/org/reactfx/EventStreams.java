...
public class EventStreams {
  ...
  private static final class Never<T> extends
    RigidObservable<Consumer<? super T>> implements EventStream<T> {
  }
  ...
  /**
   * Creates an event stream that emits an impulse on every invalidation of the
   * given observable.
   */
   ...
   * observable.
   */
  public static <O extends Observable> EventStream<O> repeatOnInvalidation(
    O observable) {
   ...
   * {@code ObservableValue} immediately for every subscriber and then on every
   * change.
   */
  ...
  public static <T> EventStream<T> nonNullValuesOf(
  ...
  public static <T> EventStream<Change<T>> changesOf(
    ObservableValue<T> observable) {
    return new EventStreamBase<Change<T>>() {
      @Override
      protected Subscription observeInputs() {
        ChangeListener<T> listener = (obs, old, val) -> emit(new Change<>(old,
          val));
        observable.addListener(listener);
        return () -> observable.removeListener(listener);
      }
   ...
   */
  public static <T> EventStream<ListChangeListener.Change<? extends T>> changesOf(
  ...
  /**
   * Use only when the subscriber does not cause {@code list} modification of
   * the underlying list.
   */
  public static <T> EventStream<ListModification<? extends T>> simpleChangesOf(
    ObservableList<T> list) {
  ...
  public static <T> EventStream<SetChangeListener.Change<? extends T>> changesOf(
    ObservableSet<T> set) {
  ...
  public static <C extends Collection<?> & Observable> EventStream<Integer> sizeOf(
    C collection) {
  ...
  private static <T> EventStream<T> create(Supplier<? extends T> computeValue,
  ...
  public static <T extends Event> EventStream<T> eventsOf(Node node,
  ...
  public static <T extends Event> EventStream<T> eventsOf(Scene scene,
    EventType<T> eventType) {
    return new EventStreamBase<T>() {
      @Override
      protected Subscription observeInputs() {
  ...
  public static <T extends Event> EventStream<T> eventsOf(MenuItem menuItem,
    EventType<T> eventType) {
    return new EventStreamBase<T>() {
      @Override
      protected Subscription observeInputs() {
  ...
  public static <T extends Event> EventStream<T> eventsOf(Window window,
    EventType<T> eventType) {
    return new EventStreamBase<T>() {
      @Override
      protected Subscription observeInputs() {
  ...
  /**
   * Returns an event stream that emits periodic <i>ticks</i>. The first tick is
   * emitted after {@code interval} amount of time has passed. The returned
   * stream may only be used on the JavaFX application thread.
   *
   * <p>
   * As with all lazily bound streams, ticks are emitted only when there is at
   * least one subscriber to the returned stream. This means that to release
   * associated resources, it suffices to unsubscribe from the returned stream.
   */
  public static EventStream<?> ticks(Duration interval) {
    return new EventStreamBase<Void>() {
      private final Timer timer = FxTimer.createPeriodic(interval, () -> emit(
        null));
  ...
  /**
   * Returns an event stream that emits periodic <i>ticks</i>. The first tick is
   * emitted at time 0. The returned stream may only be used on the JavaFX
   * application thread.
   *
   * <p>
   * As with all lazily bound streams, ticks are emitted only when there is at
  ...
  public static EventStream<?> ticks0(Duration interval) {
    return new EventStreamBase<Void>() {
      private final Timer timer = FxTimer.createPeriodic0(interval, () -> emit(
        null));
   ...
   * {@code eventThreadExecutor}. The returned stream may only be used from that
   * executor's thread.
   *
   * <p>
   * As with all lazily bound streams, ticks are emitted only when there is at
  ...
  /**
   * Returns a {@link #ticks(Duration)} EventStream whose timer restarts
   * whenever impulse emits an event.
   *
   * @param interval - the amount of time that passes until this stream emits
   *        its next tick
   * @param impulse - the EventStream that resets this EventStream's internal
   *        timer
   */
  public static EventStream<?> restartableTicks(Duration interval,
    EventStream<?> impulse) {
    return new EventStreamBase<Void>() {
      private final Timer timer = FxTimer.createPeriodic(interval, () -> emit(
        null));
      ...
      protected Subscription observeInputs() {
        timer.restart();
        return Subscription.multi(impulse.subscribe(x -> timer.restart()),
          timer::stop);
      }
    };
  }
  ...
  /**
   * Returns a {@link #ticks0(Duration)} EventStream whose timer restarts
   * whenever impulse emits an event. Note: since {@link #ticks0(Duration)} is
   * used, restarting the timer will make the returned EventStream immediately
   * emit a new tick.
   *
   * @param interval - the amount of time that passes until this stream emits
   *        its next tick
   ...
   *        timer
   */
  public static EventStream<?> restartableTicks0(Duration interval,
    EventStream<?> impulse) {
    return new EventStreamBase<Void>() {
      private final Timer timer = FxTimer.createPeriodic0(interval, () -> emit(
        null));
  ...
  /**
   * Returns a stream that, on each animation frame, emits the duration elapsed
   * since the previous animation frame, in nanoseconds.
   */
  public static EventStream<Long> animationFrames() {
    return animationTicks().accumulate(t(0L, -1L), (state, now) -> state.map((d,
      last) -> {
  ...
  /**
   * Returns an event stream that emits all the events emitted from any of the
   * {@code inputs}. The event type of the returned stream is the nearest common
   * super-type of all the {@code inputs}.
   *
  ...
  /**
   * Returns an event stream that emits all the events emitted from any of the
   * event streams in the given observable set. When an event stream is added to
   * the set, the returned stream will start emitting its events. When an event
   * stream is removed from the set, its events will no longer be emitted from
   * the returned stream.
   */
   ...
   * arbitrary element type and a function to obtain an event stream from the
   * element.
   *
   * @param set observable set of elements
   * @param f function to obtain an event stream from an element
   */
  public static <T, U> EventStream<U> merge(ObservableSet<? extends T> set,
  ...
  public static <L, R> Tuple2<EventStream<L>, EventStream<R>> fork(
    ...
    return t(stream.filterMap(Either::asLeft), stream.filterMap(
      Either::asRight));
  }

  public static <A, B> EventStream<Tuple2<A, B>> zip(EventStream<A> srcA,
    EventStream<B> srcB) {
    return new EventStreamBase<Tuple2<A, B>>() {
      ...
      protected Subscription observeInputs() {
        pocketA.clear();
        pocketB.clear();
        return Subscription.multi(srcA.subscribe(a -> {
          pocketA.set(a);
          tryEmit();
        }), srcB.subscribe(b -> {
          pocketB.set(b);
  ...
  public static <A, B, C> EventStream<Tuple3<A, B, C>> zip(EventStream<A> srcA,
    EventStream<B> srcB, EventStream<C> srcC) {
    return new EventStreamBase<Tuple3<A, B, C>>() {
      ...
      protected void tryEmit() {
        if (pocketA.hasValue() && pocketB.hasValue() && pocketC.hasValue()) {
          emit(t(pocketA.getAndClear(), pocketB.getAndClear(), pocketC
            .getAndClear()));
        }
      }
    };
  ...
  public static <A, B> EventStream<Tuple2<A, B>> combine(EventStream<A> srcA,
    EventStream<B> srcB) {
    return new EventStreamBase<Tuple2<A, B>>() {
    ...
    return new EventStreamBase<Tuple4<A, B, C, D>>() {
      ...
      void tryEmit() {
        if (pocketA.hasValue() && pocketB.hasValue() && pocketC.hasValue()
          ...
          emit(t(pocketA.get(), pocketB.get(), pocketC.get(), pocketD.get()));
        }
      }
    };
  }
  ...
  public static <A, B, C, D, E> EventStream<Tuple5<A, B, C, D, E>> combine(
    EventStream<A> srcA, EventStream<B> srcB, EventStream<C> srcC,
    EventStream<D> srcD, EventStream<E> srcE) {
    return new EventStreamBase<Tuple5<A, B, C, D, E>>() {
      ...
      void tryEmit() {
        if (pocketA.hasValue() && pocketB.hasValue() && pocketC.hasValue()
          && pocketD.hasValue() && pocketE.hasValue()) {
  ...
  public static <A, B, C, D, E, F> EventStream<Tuple6<A, B, C, D, E, F>> combine(
    EventStream<A> srcA, EventStream<B> srcB, EventStream<C> srcC,
    EventStream<D> srcD, EventStream<E> srcE, EventStream<F> srcF) {
    return new EventStreamBase<Tuple6<A, B, C, D, E, F>>() {
      ...
      void tryEmit() {
        if (pocketA.hasValue() && pocketB.hasValue() && pocketC.hasValue()
          && pocketD.hasValue() && pocketE.hasValue() && pocketF.hasValue()) {

          emit(t(pocketA.get(), pocketB.get(), pocketC.get(), pocketD.get(),
            pocketE.get(), pocketF.get()));
        }
      }
    };
  }
