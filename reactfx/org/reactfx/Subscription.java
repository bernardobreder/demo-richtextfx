...
public interface Subscription {
  ...
  /**
   * Returns a new aggregate subscription whose {@link #unsubscribe()} method
   * calls {@code unsubscribe()} on both this subscription and {@code other}
   * subscription.
   */
  ...
  /**
   * Returns a new aggregate subscription whose {@link #unsubscribe()} method
   * calls {@code unsubscribe()} on all arguments to this method.
   */
   ...
   */
  @SafeVarargs
  static <T> Subscription multi(Function<? super T, ? extends Subscription> f,
    T... elems) {
    return multi(Stream.of(elems).map(f).<Subscription> toArray(
      n -> new Subscription[n]));
  }
  ...
  static <T> Subscription multi(Function<? super T, ? extends Subscription> f,
    Collection<T> elems) {
    return multi(elems.stream().map(f).<Subscription> toArray(
      n -> new Subscription[n]));
  }

  /**
   * Dynamically subscribes to all elements of the given observable set. When an
   * element is added to the set, it is automatically subscribed to. When an
   * element is removed from the set, it is automatically unsubscribed from.
   *
   ...
   */
  static <T> Subscription dynamic(ObservableSet<T> elems,
