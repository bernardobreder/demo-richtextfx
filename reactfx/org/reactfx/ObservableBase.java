 ...
 * <li><b>Lazy binding to inputs.</b> An observable has 0 or more inputs, most
 * commonly, but not necessarily, other observables. Lazy binding to inputs
 * means that the observable observes its inputs only when it is itself being
 * observed.</li>
...
public abstract class ObservableBase<O, T> implements ProperObservable<O, T> {
  ...
  /**
   * Starts observing this observable's input(s), if any. This method is called
   * when the number of observers goes from 0 to 1. This method is called
   * <em>before</em> {@link #newObserver(Object)} is called for the first
   * observer.
   *
   * @return subscription used to stop observing inputs. The subscription is
   *         unsubscribed (i.e. input observation stops) when the number of
  ...
  /**
   * Executes action for each observer, regardless of recursion state. If
   * {@code action} throws an exception for one observer, it will not be called
   * for any subsequent observers and the exception will be propagated by this
   * method.
   *
  ...
  /**
   * Called for each new observer. Overriding this method is a convenient way
   * for subclasses to handle this event, for example to publish some initial
   * events.
   *
   * <p>
   * This method is called <em>after</em> the {@link #observeInputs()} method.
   * </p>
   */
