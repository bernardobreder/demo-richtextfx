...
public final class ConnectableEventSource<T> extends EventStreamBase<T>
  implements ConnectableEventStream<T>, ConnectableEventSink<T> {

  private MapHelper<EventStream<? extends T>, Subscription> subscriptions =
    null;
  ...
  public Subscription connectTo(EventStream<? extends T> input) {
    ...
    if (MapHelper.containsKey(subscriptions, input)) {
      throw new IllegalStateException("Already connected to event stream "
        + input);
    }
  ...
  protected final Subscription observeInputs() {
    MapHelper.replaceAll(subscriptions, (input, sub) -> subscribeToInput(
      input));
