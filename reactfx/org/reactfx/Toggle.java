...
/**
 * Observable boolean that changes value when suspended. Which boolean value is
 * the value of the base state and which is the value of the suspended state
...
public interface Toggle extends Val<Boolean>, Suspendable {
   ...
   * {@linkplain Suspendable} whose suspension causes the boolean value to
   * switch.
   *
   * @param obs boolean value that indicates suspension of {@code suspender}.
   * @param suspender Assumed to switch the value of {@code obs} when suspended
   *        and switch back when resumed, unless there are other suspenders
...
class ToggleFromVal extends ProxyVal<Boolean, Boolean> implements Toggle {
  ...
  @Override
  protected Consumer<? super Boolean> adaptObserver(
    Consumer<? super Boolean> observer) {
    return observer;
  }
