...
public interface Suspendable {
   ...
   *
   * <p>
   * In case of suspendable {@link Observable}, suspends notification delivery
   * for this observable object. Notifications produced while suspended may be
   * queued for later delivery, accumulated into a single cumulative
   * notification, or discarded completely, depending on the concrete
   * implementation.
   *
   * @return a {@linkplain Guard} instance that can be released to end
   *         suspension. In case of suspended notifications, releasing the
   *         returned {@linkplain Guard} will trigger delivery of queued or
   *         accumulated notifications, if any.
   *
   *         <p>
   ...
   *         {@code condition} and stop suspending this {@linkplain Suspendable}
   *         based on {@code condition}. If at the time of unsubscribing the
   *         returned {@linkplain Subscription} this {@linkplain Suspendable}
   *         was suspended due to {@code condition} being {@code true}, it will
   *         be resumed.
   */
  default Subscription suspendWhen(ObservableValue<Boolean> condition) {
    return new Subscription() {
      ...
      private final Subscription sub = EventStreams.valuesOf(condition)
        .subscribe(this::suspendSource).and(this::resumeSource);
   ...
   * {@linkplain Suspendable}s are suspended, in the given order. When resumed,
   * all participating {@linkplain Suspendable}s are resumed, in reverse order.
   *
   */
    ...
    switch (suspendables.length) {
      case 0:
        throw new IllegalArgumentException(
          "Must invoke with at least 1 argument");
      case 1:
...
class MultiSuspendable implements Suspendable {
  ...
  @Override
  public Guard suspend() {
    Guard[] guards = Arrays.stream(suspendables).map(g -> g.suspend()).toArray(
      n -> new Guard[n]);
