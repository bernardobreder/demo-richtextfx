...
final class Navigator<T, C extends Cell<T, ?>> extends Region implements
  ...
  public Navigator(CellListManager<T, C> cellListManager,
    CellPositioner<T, C> positioner, OrientationHelper orientation,
    Gravity gravity, SizeTracker sizeTracker) {
    ...
    this.itemsSubscription = LiveList.observeQuasiChanges(cellListManager
      .getLazyCellList(), this::itemsChanged);
  ...
  private void itemsChanged(QuasiListChange<?> ch) {
    for (QuasiListModification<?> mod : ch) {
      targetPosition = targetPosition.transformByChange(mod.getFrom(), mod
        .getRemovedSize(), mod.getAddedSize());
    }
  ...
  public void visit(MinDistanceTo targetPosition) {
    ...
    if (cell.isPresent()) {
      ...
      if ((prevVisible = positioner.lastVisibleBefore(targetPosition.itemIndex))
        .isPresent()) {
        ...
        if (cell.isPresent()) {
          ...
          placeStartOffEndMayCrop(targetPosition.itemIndex, -targetPosition.maxY
            .getValue());
        }
        else {
          placeEndOffEndMayCrop(targetPosition.itemIndex, -targetPosition.maxY
            .getValue());
        }
      }
      else if ((nextVisible = positioner.firstVisibleAfter(
        targetPosition.itemIndex + 1)).isPresent()) {
        ...
        if (cell.isPresent()) {
          ...
          placeStartAtMayCrop(targetPosition.itemIndex, -targetPosition.minY
            .getValue());
        }
        else {
          placeEndOffStartMayCrop(targetPosition.itemIndex, -targetPosition.minY
            .getValue());
        }
      }
      else {
  ...
  private void fillViewportFrom(int itemIndex) {
    ...
    while (first < last && orientation.maxY(positioner.getVisibleCell(
      first)) <= 0.0) {
      ++first;
    }
    while (last > first && orientation.minY(positioner.getVisibleCell(
      last)) >= sizeTracker.getViewportLength()) {
      --last;
    }
