...
public class VirtualFlow<T, C extends Cell<T, ?>> extends Region implements
  Virtualized {
  ...
  public static <T, C extends Cell<T, ?>> VirtualFlow<T, C> createHorizontal(
    ObservableList<T> items, Function<? super T, ? extends C> cellFactory) {
  ...
  // non-negative
  private final ReadOnlyDoubleWrapper breadthOffset = new ReadOnlyDoubleWrapper(
    0.0);
  ...
  private VirtualFlow(ObservableList<T> items,
    Function<? super T, ? extends C> cellFactory, OrientationHelper orientation,
    Gravity gravity) {
    ...
    this.sizeTracker = new SizeTracker(orientation, layoutBoundsProperty(),
      cells);
    ...
    this.navigator = new Navigator<>(cellListManager, cellPositioner,
      orientation, gravity, sizeTracker);

    getChildren().add(navigator);
    clipProperty().bind(Val.map(layoutBoundsProperty(), b -> new Rectangle(b
      .getWidth(), b.getHeight())));

    // set up bindings

    breadthPositionEstimate = Val.combine(breadthOffset, sizeTracker
      .viewportBreadthProperty(), sizeTracker.maxCellBreadthProperty(), (off,
        vpBr, totalBr) -> offsetToScrollbarPosition(off.doubleValue(), vpBr,
          totalBr));

    lengthOffsetEstimate = sizeTracker.lengthOffsetEstimateProperty();

    lengthPositionEstimate = Val.combine(lengthOffsetEstimate, sizeTracker
      .viewportLengthProperty(), sizeTracker.totalLengthEstimateProperty(), (
        off, vpLen, totalLen) -> offsetToScrollbarPosition(off, vpLen,
          totalLen)).orElseConst(0.0);
   ...
   * returned cell will be properly sized, but not properly positioned relative
   * to the cells in the viewport, unless it is itself in the viewport.
   *
   * @return Cell for the given item. The cell will be valid only until the next
   *         layout pass. It should therefore not be stored. It is intended to
  ...
  protected void layoutChildren() {
    ...
    while (true) {
      ...
      orientation.resize(navigator, oldLayoutBreadth, sizeTracker
        .getViewportLength());
      navigator.layout();
   ...
   * @return hit info containing the cell that was hit and coordinates relative
   *         to the cell. If the hit was before the cells (i.e. above a vertical
   *         flow content or left of a horizontal flow content), returns a
   *         <em>hit before cells</em> containing offset from the top left
   *         corner of the content. If the hit was after the cells (i.e. below a
   *         vertical flow content or right of a horizontal flow content),
   *         returns a <em>hit after cells</em> containing offset from the top
   *         right corner of the content of a horizontal flow or bottom left
   *         corner of the content of a vertical flow.
   */
  public VirtualFlowHit<C> hit(double x, double y) {
    ...
    if (lOff < orientation.minY(firstCell)) {
      return orientation.hitBeforeCells(bOff, lOff - orientation.minY(
        firstCell));
    }
      ...
      for (int i = firstVisible; i <= lastVisible; ++i) {
        ...
        if (lOff < orientation.maxY(cell)) {
          return orientation.cellHit(i, cell, bOff, lOff - orientation.minY(
            cell));
        }
      }
      throw new AssertionError("unreachable code");
  ...
  public void show(double viewportOffset) {
    if (viewportOffset < 0) {
      ...
      navigator.scrollTargetPositionBy(viewportOffset - sizeTracker
        .getViewportLength());
    }
    else {
  ...
  public void show(int itemIndex, Bounds region) {
    navigator.showLengthRegion(itemIndex, orientation.minY(region), orientation
      .maxY(region));
  ...
  private static double offsetToScrollbarPosition(double contentOffset,
    double viewportSize, double contentSize) {
    return contentSize > viewportSize ? contentOffset / (contentSize
      - viewportSize) * contentSize : 0;
  }
