...
final class CellListManager<T, C extends Cell<T, ?>> {
  ...
  public CellListManager(ObservableList<T> items,
    ...
    this.presentCellsSubscription = presentCells.observeQuasiModifications(
      this::presentCellsChanged);
  }
  ...
  private C cellForItem(T item) {
    ...
    EventStreams.nonNullValuesOf(node.sceneProperty()).subscribeForOne(
      scene -> {
        node.applyCss();
      });
