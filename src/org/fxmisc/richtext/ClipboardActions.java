...
public interface ClipboardActions<PS, S> extends EditActions<PS, S> {
  ...
  /**
   * Transfers the currently selected text to the clipboard, removing the
   * current selection.
   */
  default void cut() {
    copy();
  ...
  /**
   * Transfers the currently selected text to the clipboard, leaving the current
   * selection.
   */
  default void copy() {
    IndexRange selection = getSelection();
    if (selection.getLength() > 0) {
      ...
      getStyleCodecs().ifPresent(codecs -> {
        Codec<StyledDocument<PS, S>> codec = ReadOnlyStyledDocument.codec(
          codecs._1, codecs._2);
        DataFormat format = dataFormat(codec.getName());
        StyledDocument<PS, S> doc = subDocument(selection.getStart(), selection
          .getEnd());
        ...
        try {
          ...
          System.err.println("Codec error: Exception in encoding '" + codec
            .getName() + "':");
          e.printStackTrace();
        }
      });
   ...
   * replacing the current selection. If there is no selection, the content from
   * the clipboard is inserted at the current caret position.
   */
  default void paste() {
