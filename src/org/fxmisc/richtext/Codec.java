...
public interface Codec<T> {
  ...
  static final Codec<Color> COLOR_CODEC = new Codec<Color>() {
    ...
    public Color decode(DataInputStream is) throws IOException {
      return Color.color(is.readDouble(), is.readDouble(), is.readDouble(), is
        .readDouble());
    }
  ...
  static <T> Codec<Optional<T>> optionalCodec(Codec<T> elemCodec) {
    return new Codec<Optional<T>>() {
      ...
      @Override
      public void encode(DataOutputStream os, Optional<T> ot)
        throws IOException {
        if (ot.isPresent()) {
      ...
      public Optional<T> decode(DataInputStream is) throws IOException {
        return is.readBoolean() ? Optional.of(elemCodec.decode(is))
          : Optional.empty();
      }
...
interface SuperCodec<S, T extends S> extends Codec<T> {
  ...
  static <S, U extends S, T extends U> SuperCodec<S, U> upCast(
    SuperCodec<S, T> sc) {
    return (SuperCodec<S, U>) sc;
  }

  static <T> SuperCodec<Collection<T>, List<T>> collectionListCodec(
    Codec<T> elemCodec) {
    return new SuperCodec<Collection<T>, List<T>>() {
      ...
      @Override
      public void encodeSuper(DataOutputStream os, Collection<T> col)
        throws IOException {
