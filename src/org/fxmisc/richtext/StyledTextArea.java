...
/**
 * Text editing control. Accepts user input (keyboard, mouse) and provides API
 * to assign style to text ranges. It is suitable for syntax highlighting and
 * rich-text editors.
 *
 * <p>
 * Subclassing is allowed to define the type of style, e.g. inline style or
 * style classes.
 * </p>
 *
 * <p>
 * Note: Scroll bars no longer appear when the content spans outside of the
 * viewport. To add scroll bars, the area needs to be embedded in a
 * {@link VirtualizedScrollPane}. {@link AreaFactory} is provided to make this
 * more convenient.
 * </p>
 *
 ...
 * {@code onKeyPressed} handler. To add or override some keyboard shortcuts, but
 * keep the rest in place, you would combine the default event handler with a
 * new one that adds or overrides some of the default key combinations. This is
 * how to bind {@code Ctrl+S} to the {@code save()} operation:
 *
 * <pre>
 * {@code
 ...
 */
public class StyledTextArea<PS, S> extends Region implements
  TextEditingArea<PS, S>, EditActions<PS, S>, ClipboardActions<PS, S>,
  NavigationActions<PS, S>, UndoActions, TwoDimensional, Virtualized {
  ...
  private static final PseudoClass HAS_CARET = PseudoClass.getPseudoClass(
    "has-caret");

  private static final PseudoClass FIRST_PAR = PseudoClass.getPseudoClass(
    "first-paragraph");

  private static final PseudoClass LAST_PAR = PseudoClass.getPseudoClass(
    "last-paragraph");

  /*
   * ********************************************************************** * *
   * Properties * * Properties affect behavior and/or appearance of this
   * control. * * They are readable and writable by the client code and never
   * change by * other means, i.e. they contain either the default value or the
   * value * set by the client code. * *
   * **********************************************************************
   */
  ...
  // editable property
  /**
   * Indicates whether this text area can be edited by the user. Note that this
   * property doesn't affect editing through the API.
   */
  ...
  // wrapText property
  /**
   * When a run of text exceeds the width of the text region, then this property
   * indicates whether the text should wrap onto another line.
   */
  private final BooleanProperty wrapText = new SimpleBooleanProperty(this,
    "wrapText");
   ...
   * caret or selection. Use {@link #popupAlignmentProperty()} to specify how
   * the popup should be positioned relative to the caret or selection. Use
   * {@link #popupAnchorOffsetProperty()} or
  ...
  /**
   * Specifies further offset (in pixels) of the popup window from the position
   * specified by {@link #popupAlignmentProperty()}.
   *
   * <p>
   * If {@link #popupAnchorAdjustmentProperty()} is also specified, then it
   * overrides the offset set by this property.
   */
  private final ObjectProperty<Point2D> popupAnchorOffset =
    new SimpleObjectProperty<>();

  public void setPopupAnchorOffset(Point2D offset) {
    popupAnchorOffset.set(offset);
  }

  public Point2D getPopupAnchorOffset() {
    return popupAnchorOffset.get();
  }

  public ObjectProperty<Point2D> popupAnchorOffsetProperty() {
    return popupAnchorOffset;
  }

  /**
   * Specifies how to adjust the popup window's anchor point. The given operator
   * is invoked with the screen position calculated according to
   * {@link #popupAlignmentProperty()} and should return a new screen position.
   * This position will be used as the popup window's anchor point.
   *
   * <p>
  ...
  public void setPopupAnchorAdjustment(UnaryOperator<Point2D> f) {
    popupAnchorAdjustment.set(f);
  }

  public UnaryOperator<Point2D> getPopupAnchorAdjustment() {
    return popupAnchorAdjustment.get();
  }

  public ObjectProperty<UnaryOperator<Point2D>> popupAnchorAdjustmentProperty() {
    return popupAnchorAdjustment;
  }

  /**
   * Defines where the popup window given in {@link #popupWindowProperty()} is
   * anchored, i.e. where its anchor point is positioned. This position can
   * further be adjusted by {@link #popupAnchorOffsetProperty()} or
   * {@link #popupAnchorAdjustmentProperty()}.
   */
  private final ObjectProperty<PopupAlignment> popupAlignment =
    new SimpleObjectProperty<>(CARET_TOP);

  public void setPopupAlignment(PopupAlignment pos) {
    popupAlignment.set(pos);
  }
   ...
   * {@link MouseOverTextEvent} of type {@code MOUSE_OVER_TEXT_BEGIN} is fired
   * on this text area. When set to {@code null}, no {@code MouseOverTextEvent}s
   * are fired on this text area.
   *
   * <p>
  ...
  public void setMouseOverTextDelay(Duration delay) {
    mouseOverTextDelay.set(delay);
  }

  public Duration getMouseOverTextDelay() {
    return mouseOverTextDelay.get();
  }
  ...
  /**
   * Defines how to handle an event in which the user has selected some text,
   * dragged it to a new location within the area, and released the mouse at
   * some character {@code index} within the area.
   *
   * <p>
   * By default, this will relocate the selected text to the character index
   * where the mouse was released. To override it, use
   * {@link #setOnSelectionDrop(IntConsumer)}.
   */
  private Property<IntConsumer> onSelectionDrop = new SimpleObjectProperty<>(
    this::moveSelectedText);

  public final void setOnSelectionDrop(IntConsumer consumer) {
    onSelectionDrop.setValue(consumer);
  }

  public final IntConsumer getOnSelectionDrop() {
    return onSelectionDrop.getValue();
  }
  ...
  public void setParagraphGraphicFactory(IntFunction<? extends Node> factory) {
    paragraphGraphicFactory.set(factory);
  }

  public IntFunction<? extends Node> getParagraphGraphicFactory() {
    return paragraphGraphicFactory.get();
  }
  ...
  public BooleanProperty useInitialStyleForInsertionProperty() {
    return model.useInitialStyleForInsertionProperty();
  }

  public void setUseInitialStyleForInsertion(boolean value) {
    model.setUseInitialStyleForInsertion(value);
  }

  public boolean getUseInitialStyleForInsertion() {
    return model.getUseInitialStyleForInsertion();
  }
   ...
   */
  public void setStyleCodecs(Codec<PS> paragraphStyleCodec,
  ...
  /**
   * The <em>estimated</em> scrollX value. This can be set in order to scroll
   * the content. Value is only accurate when area does not wrap lines and uses
   * the same font size throughout the entire area.
   */
  @Override
  ...
  public double getEstimatedScrollX() {
    return virtualFlow.estimatedScrollXProperty().getValue();
  }

  public void setEstimatedScrollX(double value) {
    virtualFlow.estimatedScrollXProperty().setValue(value);
  }

  /**
   * The <em>estimated</em> scrollY value. This can be set in order to scroll
   * the content. Value is only accurate when area does not wrap lines and uses
   * the same font size throughout the entire area.
   */
  @Override
  ...
  public double getEstimatedScrollY() {
    return virtualFlow.estimatedScrollYProperty().getValue();
  }

  public void setEstimatedScrollY(double value) {
    virtualFlow.estimatedScrollYProperty().setValue(value);
  }

  /*
   * ********************************************************************** * *
   * Observables * * Observables are "dynamic" (i.e. changing) characteristics
   * of this * control. They are not directly settable by the client code, but
   * change * in response to user input and/or API actions. * *
   * **********************************************************************
   */
  ...
  public final ObservableValue<Integer> caretPositionProperty() {
    return model.caretPositionProperty();
  }
  ...
  public final ObservableValue<IndexRange> selectionProperty() {
    return model.selectionProperty();
  }
  ...
  public ObservableBooleanValue beingUpdatedProperty() {
    return model.beingUpdatedProperty();
  }

  public boolean isBeingUpdated() {
    return model.isBeingUpdated();
  }

  // total width estimate
  /**
   * The <em>estimated</em> width of the entire document. Accurate when area
   * does not wrap lines and uses the same font size throughout the entire area.
   * Value is only supposed to be <em>set</em> by the skin, not the user.
   */
  @Override
  ...
  public double getTotalWidthEstimate() {
    return virtualFlow.totalWidthEstimateProperty().getValue();
  }

  // total height estimate
  /**
   * The <em>estimated</em> height of the entire document. Accurate when area
   * does not wrap lines and uses the same font size throughout the entire area.
   * Value is only supposed to be <em>set</em> by the skin, not the user.
   */
  @Override
  ...
  public double getTotalHeightEstimate() {
    return virtualFlow.totalHeightEstimateProperty().getValue();
  }
   ...
   */
  protected final S getInitialTextStyle() {
    return model.getInitialTextStyle();
  }
  ...
  protected final PS getInitialParagraphStyle() {
    return model.getInitialParagraphStyle();
  }
  ...
  /**
   * Indicates whether style should be preserved on undo/redo, copy/paste and
   * text move. TODO: Currently, only undo/redo respect this flag.
   */
  protected final boolean isPreserveStyle() {
    return model.isPreserveStyle();
  }
   ...
   *        specified (yet).
   * @param applyStyle function that, given a {@link Text} node and a style,
   *        applies the style to the text node. This function is used by the
   *        default skin to apply style to text nodes.
   ...
   */
  public StyledTextArea(PS initialParagraphStyle,
    BiConsumer<TextFlow, PS> applyParagraphStyle, S initialTextStyle,
  ...
  public StyledTextArea(PS initialParagraphStyle,
    ...
    this(initialParagraphStyle, applyParagraphStyle, initialTextStyle,
      applyStyle, new EditableStyledDocument<>(initialParagraphStyle,
        initialTextStyle), preserveStyle);
  }

  /**
   * The same as {@link #StyledTextArea(Object, BiConsumer, Object, BiConsumer)}
   * except that this constructor can be used to create another
   * {@code StyledTextArea} object that shares the same
   * {@link EditableStyledDocument}.
   */
  public StyledTextArea(PS initialParagraphStyle,
    BiConsumer<TextFlow, PS> applyParagraphStyle, S initialTextStyle,
    BiConsumer<? super TextExt, S> applyStyle,
  ...
  public StyledTextArea(PS initialParagraphStyle,
    ...
    getStylesheets().add(StyledTextArea.class.getResource(
      "styled-text-area.css").toExternalForm());

    // keeps track of currently used non-empty cells
    @SuppressWarnings("unchecked")
    ObservableSet<ParagraphBox<PS, S>> nonEmptyCells = FXCollections
      .observableSet();

    // Initialize content
    virtualFlow = VirtualFlow.createVertical(getParagraphs(), par -> {
      Cell<Paragraph<PS, S>, ParagraphBox<PS, S>> cell = createCell(par,
        applyStyle, applyParagraphStyle);
    ...
    IntUnaryOperator cellLength = i -> virtualFlow.getCell(i).getNode()
      .getLineCount();
    ...
    // need to reposition popup even when caret hasn't moved, but selection has
    // changed (been deselected)
    EventStream<?> caretDirty = merge(caretPosDirty, paragraphsDirty,
      selectionDirty);
    subscribeTo(caretDirty, x -> requestFollowCaret());

    // whether or not to animate the caret
    BooleanBinding blinkCaret = focusedProperty().and(editableProperty()).and(
      disabledProperty().not());
    manageBinding(blinkCaret);

    // The caret is visible in periodic intervals,
    // but only when blinkCaret is true.
    caretVisible = EventStreams.valuesOf(blinkCaret).flatMap(blink -> blink
      ? booleanPulse(Duration.ofMillis(500), caretDirty)
      : EventStreams.valuesOf(Val.constant(false))).toBinding(false);
    ...
    Val<UnaryOperator<Point2D>> userOffset = Val.map(
      popupAnchorOffsetProperty(), offset -> anchor -> anchor.add(offset));
    _popupAnchorAdjustment = Val.orElse(popupAnchorAdjustmentProperty(),
      userOffset).orElseConst(UnaryOperator.identity());

    // dispatch MouseOverTextEvents when mouseOverTextDelay is not null
    EventStreams.valuesOf(mouseOverTextDelayProperty()).flatMap(
      delay -> delay != null ? mouseOverTextEvents(nonEmptyCells, delay)
        : EventStreams.never()).subscribe(evt -> Event.fireEvent(this, evt));
  ...
  /*
   * ********************************************************************** * *
   * Queries * * Queries are parameterized observables. * *
   * **********************************************************************
   */

  /**
   * Returns caret bounds relative to the viewport, i.e. the visual bounds of
   * the embedded VirtualFlow.
   */
  Optional<Bounds> getCaretBounds() {
    return virtualFlow.getCellIfVisible(getCurrentParagraph()).map(c -> {
  ...
  CharacterHit hit(ParagraphBox.CaretOffsetX x,
  ...
  CharacterHit hit(ParagraphBox.CaretOffsetX x, double y) {
    VirtualFlowHit<Cell<Paragraph<PS, S>, ParagraphBox<PS, S>>> hit =
      virtualFlow.hit(0.0, y);
  ...
  /**
   * Returns the current line as a two-level index. The major number is the
   * paragraph index, the minor number is the line number within the paragraph.
   *
   * <p>
   * This method has a side-effect of bringing the current paragraph to the
   * viewport if it is not already visible.
   */
  TwoDimensional.Position currentLine() {
    int parIdx = getCurrentParagraph();
    Cell<Paragraph<PS, S>, ParagraphBox<PS, S>> cell = virtualFlow.getCell(
      parIdx);
  ...
  /**
   * Returns the style of the character with the given index. If {@code index}
   * points to a line terminator character, the last style used in the paragraph
   * terminated by that line terminator is returned.
   */
   ...
   * immediately preceding {@code position}, except when {@code position} points
   * to a paragraph boundary, in which case it is the style at the beginning of
   * the latter paragraph.
   *
   * <p>
   * In other words, most of the time {@code getStyleAtPosition(p)} is
   * equivalent to {@code getStyleOfChar(p-1)}, except when {@code p} points to
   * a paragraph boundary, in which case it is equivalent to
   ...
   * range preceding {@code position} is returned. If {@code position} points to
   * a boundary between two paragraphs, then the first styled range of the
   * latter paragraph is returned.
   */
   ...
   * paragraph. If {@code index} is beyond the end of the paragraph, the style
   * at the end of line is returned. If {@code index} is negative, it is the
   * same as if it was 0.
   */
  ...
  /**
   * Returns the style at the given position in the given paragraph. This is
   * equivalent to {@code getStyleOfChar(paragraph, position-1)}.
   */
  ...
  /**
   * Returns the range of homogeneous style that includes the given position in
   * the given paragraph. If {@code position} points to a boundary between two
   * styled ranges, then the range preceding {@code position} is returned.
   */
  ...
  /*
   * ********************************************************************** * *
   * Actions * * Actions change the state of this control. They typically cause
   * a * change of one or more observables and/or produce an event. * *
   * **********************************************************************
   */
   ...
   * but the actual implementation is more efficient.
   */
  public void setStyleSpans(int paragraph, int from,
  ...
  /**
   * Resets the style of the given range in the given paragraph to the initial
   * style.
   */
  ...
  private Cell<Paragraph<PS, S>, ParagraphBox<PS, S>> createCell(
    Paragraph<PS, S> paragraph, BiConsumer<? super TextExt, S> applyStyle,
    ...
    Val<Boolean> hasCaret = Val.combine(box.indexProperty(),
      currentParagraphProperty(), (bi, cp) -> bi.intValue() == cp.intValue());

    Subscription hasCaretPseudoClass = hasCaret.values().subscribe(value -> box
      .pseudoClassStateChanged(HAS_CARET, value));
    Subscription firstParPseudoClass = box.indexProperty().values().subscribe(
      idx -> box.pseudoClassStateChanged(FIRST_PAR, idx == 0));
    Subscription lastParPseudoClass = EventStreams.combine(box.indexProperty()
      .values(), getParagraphs().sizeProperty().values()).subscribe(in -> in
        .exec((i, n) -> box.pseudoClassStateChanged(LAST_PAR, i == n - 1)));

    // caret is visible only in the paragraph with the caret
    Val<Boolean> cellCaretVisible = hasCaret.flatMap(x -> x ? caretVisible
      : Val.constant(false));
  ...
  private EventStream<MouseOverTextEvent> mouseOverTextEvents(
    ObservableSet<ParagraphBox<PS, S>> cells, Duration delay) {
    return merge(cells, c -> c.stationaryIndices(delay).map(e -> e.unify(l -> l
      .map((pos, charIdx) -> MouseOverTextEvent.beginAt(c.localToScreen(pos),
        getParagraphOffset(c.getIndex()) + charIdx)), r -> MouseOverTextEvent
          .end())));
  }
  ...
  private void positionPopup(PopupWindow popup, PopupAlignment alignment,
    UnaryOperator<Point2D> adjustment) {
    Optional<Bounds> bounds = null;
    switch (alignment.getAnchorObject()) {
      case CARET:
        bounds = getCaretBoundsOnScreen();
        break;
      case SELECTION:
        bounds = getSelectionBoundsOnScreen();
        break;
    }
    bounds.ifPresent(b -> {
      double x = 0, y = 0;
  ...
  private Optional<Bounds> getCaretBoundsOnScreen() {
    return virtualFlow.getCellIfVisible(getCurrentParagraph()).map(c -> c
      .getNode().getCaretBoundsOnScreen());
  }
  ...
  private Optional<Bounds> getSelectionBoundsOnScreen() {
    ...
    Bounds[] bounds = virtualFlow.visibleCells().stream().map(c -> c.getNode()
      .getSelectionBoundsOnScreen()).filter(Optional::isPresent).map(
        Optional::get).toArray(Bounds[]::new);

    if (bounds.length == 0) {
      return Optional.empty();
    }
    double minX = Stream.of(bounds).mapToDouble(Bounds::getMinX).min()
      .getAsDouble();
    double maxX = Stream.of(bounds).mapToDouble(Bounds::getMaxX).max()
      .getAsDouble();
    double minY = Stream.of(bounds).mapToDouble(Bounds::getMinY).min()
      .getAsDouble();
    double maxY = Stream.of(bounds).mapToDouble(Bounds::getMaxY).max()
      .getAsDouble();
  ...
  private static Bounds extendLeft(Bounds b, double w) {
    if (w == 0) {
      ...
      return new BoundingBox(b.getMinX() - w, b.getMinY(), b.getWidth() + w, b
        .getHeight());
    }
  }

  private static EventStream<Boolean> booleanPulse(Duration duration,
    EventStream<?> restartImpulse) {
    EventStream<?> ticks = EventStreams.restartableTicks(duration,
      restartImpulse);
    return StateMachine.init(false).on(restartImpulse.withDefaultEvent(null))
      .transition((state, impulse) -> true).on(ticks).transition((state,
        tick) -> !state).toStateStream();
  }
}
