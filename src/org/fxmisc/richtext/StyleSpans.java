...
public interface StyleSpans<S> extends Iterable<StyleSpan<S>>, TwoDimensional {
   ...
   * number of {@code StyleSpan}s and the {@code StyleSpan}s are pairwise equal.
   */
  @Override
  ...
  default StyleSpans<S> append(StyleSpan<S> span) {
    ...
    if (Objects.equals(myLastSpan.getStyle(), span.getStyle())) {
      StyleSpan<S> newLastSpan = new StyleSpan<>(span.getStyle(), myLastSpan
  ...
  default StyleSpans<S> prepend(StyleSpan<S> span) {
    ...
    if (Objects.equals(span.getStyle(), myFirstSpan.getStyle())) {
      StyleSpan<S> newFirstSpan = new StyleSpan<>(span.getStyle(), span
        .getLength() + myFirstSpan.getLength());
  ...
  default StyleSpans<S> subView(int from, int to) {
    ...
    Position end = to > from ? start.offsetBy(to - from, Backward) : start;
  ...
  default StyleSpans<S> concat(StyleSpans<S> that) {
    ...
    if (Objects.equals(myLast.getStyle(), theirFirst.getStyle())) {
      ...
      builder.add(myLast.getStyle(), myLast.getLength() + theirFirst
        .getLength());
      for (int i = 1; i < n2; ++i) {
  ...
  /**
   * Returns a new {@code StyleSpans} object that has the same total length as
   * this StyleSpans and style of every span is mapped by the given function.
   * Adjacent style spans whose style mapped to the same value are merged into
   * one. As a consequence, the returned StyleSpans might have fewer style spans
   * than this StyleSpans.
   *
