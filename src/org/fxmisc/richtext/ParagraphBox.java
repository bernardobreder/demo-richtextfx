...
class ParagraphBox<PS, S> extends Region {
  ...
  /**
   * An opaque class representing horizontal caret offset. Although it is just a
   * wrapper around double, its purpose is to increase type safety.
   */
  ...
  ParagraphBox(Paragraph<PS, S> par,
    ...
    graphic = Val.combine(graphicFactory, this.index, (f, i) -> f != null ? f
      .apply(i) : null);
  ...
  public String toString() {
    return graphic.isPresent() ? "[#|" + text.getParagraph() + "]"
      : "[" + text.getParagraph() + "]";
  }

  public Property<Boolean> caretVisibleProperty() {
    return text.caretVisibleProperty();
  }

  public Property<Paint> highlightFillProperty() {
    return text.highlightFillProperty();
  }
  ...
  public Var<Integer> caretPositionProperty() {
    return text.caretPositionProperty();
  }

  public Property<IndexRange> selectionProperty() {
    return text.selectionProperty();
  }
  ...
  public EventStream<Either<Tuple2<Point2D, Integer>, Object>> stationaryIndices(
    Duration delay) {
    EventStream<Either<Point2D, Void>> stationaryEvents =
      new MouseStationaryHelper(this).events(delay);
    EventStream<Tuple2<Point2D, Integer>> hits = stationaryEvents.filterMap(
      Either::asLeft).filterMap(p -> {
    ...
    EventStream<?> stops = stationaryEvents.filter(Either::isRight).map(
      Either::getRight);
  ...
  protected double computePrefWidth(double ignoredHeight) {
    ...
    return wrapText.get() ? 0 // return 0, VirtualFlow will size it to its width
                              // anyway
      : getGraphicPrefWidth() + text.prefWidth(-1) + insets.getLeft() + insets
        .getRight();
  }
  ...
  protected double computePrefHeight(double width) {
    Insets insets = getInsets();
    double overhead = getGraphicPrefWidth() - insets.getLeft() - insets
      .getRight();
    return text.prefHeight(width - overhead) + insets.getTop() + insets
      .getBottom();
  }
  ...
  protected void layoutChildren() {
    ...
    graphic.ifPresent(g -> g.resizeRelocate(graphicOffset.get(), 0,
      graphicWidth, h));
  }
