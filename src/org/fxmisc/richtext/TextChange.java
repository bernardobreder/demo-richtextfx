...
public abstract class TextChange<S, Self extends TextChange<S, Self>> {
  ...
  /**
   * Merges this change with the given change, if possible. This change is
   * considered to be the former and the given change is considered to be the
   * latter. Changes can be merged if either
   * <ul>
   ...
   * @return a new merged change if changes can be merged, {@code null}
   *         otherwise.
   */
  public Optional<Self> mergeWith(Self latter) {
    ...
    else if (latter.position + latter.removedLength() == this.position + this
      .insertedLength()) {
      if (this.position <= latter.position) {
        S addedText = concat(sub(this.inserted, 0, latter.position
          - this.position), latter.inserted);
        return Optional.of(create(this.position, this.removed, addedText));
      }
      else {
        S removedText = concat(sub(latter.removed, 0, this.position
          - latter.position), this.removed);
        return Optional.of(create(latter.position, removedText,
          latter.inserted));
      }
    }
    else {
  ...
  public boolean equals(Object other) {
    if (other instanceof TextChange) {
      ...
      return Objects.equals(this.position, that.position) && Objects.equals(
        this.removed, that.removed) && Objects.equals(this.inserted,
          that.inserted);
    }
    else {
      return false;
