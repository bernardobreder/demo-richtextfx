...
public interface NavigationActions<PS, S> extends TextEditingArea<PS, S> {
  ...
  /**
   * Moves the caret to the position indicated by {@code pos}. Based on the
   * selection policy, the selection is either <em>cleared</em> (i.e. anchor is
   * set to the same position as caret), <em>adjusted</em> (i.e. anchor is not
   * moved at all), or <em>extended</em> (i.e. {@code pos} becomes the new caret
   * and, if {@code pos} points outside the current selection, the far end of
   * the current selection becomes the anchor.
   */
  ...
  /**
   * Moves the caret backward one char in the text. Based on the given selection
   * policy, anchor either moves with the caret, stays put, or moves to the
   * former caret position.
   */
  default void previousChar(SelectionPolicy selectionPolicy) {
    if (getCaretPosition() > 0) {
      int newCaretPos = Character.offsetByCodePoints(getText(),
        getCaretPosition(), -1);
  ...
  /**
   * Moves the caret forward one char in the text. Based on the given selection
   * policy, anchor either moves with the caret, stays put, or moves to the
   * former caret position.
   */
  ...
  /**
   * Skips n number of word boundaries backwards. Based on the given selection
   * policy, anchor either moves with the caret, stays put, or moves to the
   * former caret position.
   */
  ...
  /**
   * Skips n number of word boundaries forward. Based on the given selection
   * policy, anchor either moves with the caret, stays put, or moves to the
   * former caret position.
   */
