...
public abstract class MouseOverTextEvent extends Event {
  ...
  public static final EventType<MouseOverTextEvent> ANY = new EventType<>(
    Event.ANY, "MOUSE_OVER_TEXT_ANY");
  ...
  public static final MouseOverTextEvent beginAt(Point2D screenPos,
    int charIdx) {
  ...
  private static class Begin extends MouseOverTextEvent {
    ...
    public Point2D getScenePosition() {
      ...
      return screenPos.subtract(scene.getX() + scene.getWindow().getX(), scene
        .getY() + scene.getWindow().getY());
    }
  ...
  private MouseOverTextEvent(Object source, EventTarget target,
    EventType<? extends MouseOverTextEvent> type) {
    super(source, target, type);
  }

  /**
   * For {@link #MOUSE_OVER_TEXT_BEGIN} events returns the position at which the
   * mouse is standing still, relative to the node on which the event handler
   * was registered.
   *
   * <p>
   * For {@link #MOUSE_OVER_TEXT_END} events returns {@code null}
   */
  ...
  /**
   * Returns index of the character that the mouse stopped over if this event is
   * a {@link #MOUSE_OVER_TEXT_BEGIN} event and -1 if this event is a
