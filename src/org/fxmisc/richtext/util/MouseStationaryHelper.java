...
public class MouseStationaryHelper {
  ...
  public EventStream<Either<Point2D, Void>> events(Duration delay) {
    ...
    EventStream<Point2D> stationaryPositions = mouseEvents.successionEnds(delay)
      .filter(e -> e.getEventType() == MOUSE_MOVED).map(e -> new Point2D(e
        .getX(), e.getY()));
  ...
  public void install(Duration delay) {
    ...
    installed = events(delay).<Event> map(either -> either.unify(
      ...
      stop -> MouseStationaryEvent.end())).subscribe(evt -> Event.fireEvent(
        node, evt));
  }
