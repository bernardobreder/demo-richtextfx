...
public interface EditActions<PS, S> extends TextEditingArea<PS, S> {
  ...
  /**
   * Replaces the selection with the given replacement String. If there is no
   * selection, then the replacement text is simply inserted at the current
   * caret position. If there was a selection, then the selection is cleared and
   * the given replacement text is inserted.
   */
  ...
  /**
   * Replaces the selection with the given rich-text replacement. If there is no
   * selection, then the replacement is simply inserted at the current caret
   * position. If there was a selection, then the selection is cleared and the
   * given replacement text is inserted.
   */
  ...
  default void moveSelectedText(int pos) {
    ...
    if (pos >= sel.getStart() && pos <= sel.getEnd()) {
      ...
      StyledDocument<PS, S> text = this.subDocument(sel.getStart(), sel
        .getEnd());
      if (pos > sel.getEnd()) {
        pos -= sel.getLength();
      }
