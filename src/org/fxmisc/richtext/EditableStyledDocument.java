...
/**
 * Content model for {@link StyledTextArea}. Implements edit operations on
 * styled text, but not worrying about additional aspects such as caret or
 * selection.
 */
final class EditableStyledDocument<PS, S> extends
  StyledDocumentBase<PS, S, ObservableList<Paragraph<PS, S>>> {

  /*
   * ********************************************************************** * *
   * Observables * * Observables are "dynamic" (i.e. changing) characteristics
   * of an object.* They are not directly settable by the client code, but
   ...
   */
  private final StringBinding text = Bindings.createStringBinding(() -> getText(
    0, length()));
   ...
   */
  private final SuspendableVar<Integer> length = Var.newSimpleVar(0)
    .suspendable();
  ...
  {
    EventStream<String> removedText = EventStreams.zip(textChangePosition,
      textRemovalEnd).map(t2 -> t2.map(this::getText));
    EventStream<Integer> changePosition = EventStreams.merge(textChangePosition,
      styleChangePosition);
    EventStream<Integer> removalEnd = EventStreams.merge(textRemovalEnd,
      styleChangeEnd);
    EventStream<StyledDocument<PS, S>> removedDocument = EventStreams.zip(
      changePosition, removalEnd).map(t2 -> t2.map(this::subSequence));
    ...
    EventStream<StyledDocument<PS, S>> insertedDocument = EventStreams.merge(
      this.insertedDocument, changePosition.emitBothOnEach(insertionEnd).map(
        t2 -> t2.map(this::subSequence)));

    plainTextChanges = EventStreams.zip(textChangePosition, removedText,
      insertedText).filter(t3 -> t3.map((pos, removed, inserted) -> !removed
        .equals(inserted))).map(t3 -> t3.map(PlainTextChange::new));

    richChanges = EventStreams.zip(changePosition, removedDocument,
      insertedDocument).filter(t3 -> t3.map((pos, removed, inserted) -> !removed
        .equals(inserted))).map(t3 -> t3.map(RichTextChange::new));
  }
  ...
  /*
   * ********************************************************************** * *
   * Actions * * Actions change the state of the object. They typically cause a
   * change * of one or more observables and/or produce an event. * *
   * **********************************************************************
   */

  /**
   * The style of the inserted text will be the style at position {@code start}
   * in the current document.
   */
  public void replaceText(int start, int end, String text) {
    StyledDocument<PS, S> doc = ReadOnlyStyledDocument.fromString(text,
      getParagraphStyleAtPosition(start), getStyleAtPosition(start));
  ...
  public void replace(int start, int end, StyledDocument<PS, S> replacement) {
    ...
    Paragraph<PS, S> lastPar = paragraphs.get(lastParIdx).subSequence(
      lastParTo);
    ...
    int replacementLength = replacementPars.stream().mapToInt(Paragraph::length)
      .sum() + replacementPars.size() - 1;
    int newLength = length.getValue() - (end - start) + replacementLength;
    length.suspendWhile(() -> { // don't publish length change until text is
                                // invalidated
    ...
    StyledDocument<PS, S> doc = replacement instanceof ReadOnlyStyledDocument
      ? replacement
  ...
  public void setStyle(int from, int to, S style) {
    ...
    try (Guard commitOnClose = beginStyleChange(from, to)) {
      ...
      Position end = to == from ? start : start.offsetBy(to - from, Backward);
      ...
      if (firstParIdx == lastParIdx) {
        ...
        restyledPars.add(firstPar.restyle(firstParFrom, firstPar.length(),
          style));
  ...
  public void setStyleSpans(int from, StyleSpans<? extends S> styleSpans) {
    ...
    try (Guard commitOnClose = beginStyleChange(from, from + len)) {
      ...
      if (firstParIdx == lastParIdx) {
        ...
        Position spansTo = spansFrom.offsetBy(firstPar.length() - firstParFrom,
          Backward);
        Paragraph<PS, S> q = firstPar.restyle(firstParFrom, styleSpans.subView(
          spansFrom, spansTo));
        if (q != firstPar) {
          paragraphs.set(firstParIdx, q);
        }
  ...
  private int terminatorLengthToSkip(Position pos) {
    ...
    int skipSum = 0;
    while (pos.getMinor() == par.length() && pos.getMajor() < paragraphs.size()
      - 1) {
      skipSum += 1;
  ...
  private int terminatorLengthToTrim(Position pos) {
    ...
    while (pos.getMinor() > parLen) {
      ...
      trimSum += 1;
      pos = pos.offsetBy(-1, Backward); // may jump to the end of previous
                                        // paragraph, if parLen was 0
  ...
  private List<Paragraph<PS, S>> join(Paragraph<PS, S> first,
    List<Paragraph<PS, S>> middle, Paragraph<PS, S> last) {
    int m = middle.size();
    if (m == 0) {
  ...
  private void setAll(int startIdx, int endIdx,
    Collection<Paragraph<PS, S>> pars) {
    if (startIdx > 0 || endIdx < paragraphs.size()) {
      paragraphs.subList(startIdx, endIdx).clear(); // note that paragraphs
                                                    // remains non-empty at all
                                                    // times
