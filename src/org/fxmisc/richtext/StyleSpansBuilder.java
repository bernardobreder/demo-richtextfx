...
public class StyleSpansBuilder<S> {
  ...
  static <S> StyleSpans<S> overlay(StyleSpans<S> s1, StyleSpans<S> s2,
    BiFunction<? super S, ? super S, ? extends S> f) {

    StyleSpansBuilder<S> acc = new StyleSpansBuilder<>(s1.getSpanCount() + s2
      .getSpanCount());

    Iterator<StyleSpan<S>> t1 = s1.iterator();
    Iterator<StyleSpan<S>> t2 = s2.iterator();

    StyleSpan<S> h1 = t1.next(); // remember that all StyleSpans have at least
                                 // one StyleSpan
    StyleSpan<S> h2 = t2.next(); // remember that all StyleSpans have at least
                                 // one StyleSpan
  ...
  public StyleSpansBuilder<S> addAll(
    Collection<? extends StyleSpan<S>> styleSpans) {
    return addAll(styleSpans, styleSpans.size());
  }

  public StyleSpansBuilder<S> addAll(
    Iterable<? extends StyleSpan<S>> styleSpans, int sizeHint) {
    spans.ensureCapacity(spans.size() + sizeHint);
    return addAll(styleSpans);
  }

  public StyleSpansBuilder<S> addAll(
    Iterable<? extends StyleSpan<S>> styleSpans) {
    ensureNotCreated();
  ...
  public StyleSpansBuilder<S> addAll(
    Iterator<? extends StyleSpan<S>> styleSpans) {
    ensureNotCreated();
    ...
    if (spans.isEmpty()) {
      ...
      if (spans.size() == 1 && spans.get(0).getLength() == 0) {
        ...
        if (prev.getStyle().equals(span.getStyle())) {
          spans.set(spans.size() - 1, new StyleSpan<>(span.getStyle(), prev
            .getLength() + span.getLength()));
        }
        else {
          spans.add(span);
        }
  ...
  private void ensureNotCreated() {
    if (created) {
      throw new IllegalStateException(
        "Cannot reus StyleRangesBuilder after StyleRanges have been created.");
    }
  }
}
...
abstract class StyleSpansBase<S> implements StyleSpans<S> {
  protected final TwoLevelNavigator navigator = new TwoLevelNavigator(
    this::getSpanCount, i -> getStyleSpan(i).getLength());
...
class UpdatedSpans<S> extends StyleSpansBase<S> {
  ...
  public int length() {
    if (length == -1) {
      length = original.length() - original.getStyleSpan(index).getLength()
        + update.getLength();
    }
    return length;
  }
