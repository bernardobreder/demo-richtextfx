 ...
 *
 * Defines the core methods. Other interfaces define default higher-level
 * methods implemented on top of the core methods.
 *
...
public interface TextEditingArea<PS, S> {
  ...
  /**
   * Rich-text content of this text-editing area. The returned document is
   * immutable, it does not reflect subsequent edits of this text-editing area.
   */
   ...
   *
   * Most of the time, caret is at the boundary of the selection (if there is
   * any selection). However, there are circumstances when the caret is
   * positioned inside or outside the selected text. For example, when the user
   * is dragging the selected text, the caret moves with the cursor to point at
   * the position where the selected text moves upon release.
   */
  ...
  /**
   * The anchor of the selection. If there is no selection, this is the same as
   * caret position.
   */
  int getAnchor();
   ...
   * The selection range.
   *
   * One boundary is always equal to anchor, and the other one is most of the
   * time equal to caret position.
   */
  ...
  /**
   * Positions the anchor and caretPosition explicitly, effectively creating a
   * selection.
   */
   ...
   * @param text The text to put in place of the deleted range. It must not be
   *        null.
   */
  ...
  /**
   * Positions only the caret. Doesn't move the anchor and doesn't change the
   * selection. Can be used to achieve the special case of positioning the caret
   * outside or inside the selection, as opposed to always being at the
   * boundary. Use with care.
   */
