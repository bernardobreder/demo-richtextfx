 ...
 */
class StyledTextAreaModel<PS, S> implements TextEditingArea<PS, S>,
  EditActions<PS, S>, NavigationActions<PS, S>, UndoActions, TwoDimensional {
  ...
  public void setUndoManager(UndoManagerFactory undoManagerFactory) {
    undoManager.close();
    undoManager = preserveStyle ? createRichUndoManager(undoManagerFactory)
  ...
  public void setUseInitialStyleForInsertion(boolean value) {
    useInitialStyleForInsertion.set(value);
  }

  public boolean getUseInitialStyleForInsertion() {
    return useInitialStyleForInsertion.get();
  }
  ...
  private final SuspendableVal<Integer> caretPosition = internalCaretPosition
    .suspendable();
  ...
  // selection anchor
  private final SuspendableVar<Integer> anchor = Var.newSimpleVar(0)
    .suspendable();
  ...
  // selection
  private final Var<IndexRange> internalSelection = Var.newSimpleVar(
    EMPTY_RANGE);

  private final SuspendableVal<IndexRange> selection = internalSelection
    .suspendable();
  ...
  public final int getCurrentParagraph() {
    return currentParagraph.getValue();
  }
  ...
  /**
   * Usually used to create another area (View) that shares the same document
   * (Model).
   *
   * @return this area's {@link EditableStyledDocument}
   */
  protected final EditableStyledDocument<PS, S> getContent() {
    return content;
  }
  ...
  public StyledTextAreaModel(PS initialParagraphStyle, S initialTextStyle,
    ...
    this(initialParagraphStyle, initialTextStyle, new EditableStyledDocument<>(
      initialParagraphStyle, initialTextStyle), preserveStyle);
  }

  /**
   * The same as {@link #StyledTextAreaModel(Object, Object)} except that this
   * constructor can be used to create another {@code StyledTextArea} object
   * that shares the same {@link EditableStyledDocument}.
   */
  public StyledTextAreaModel(PS initialParagraphStyle, S initialTextStyle,
  ...
  public StyledTextAreaModel(PS initialParagraphStyle, S initialTextStyle,
    ...
    subscribeTo(content.plainTextChanges(), plainTextChange -> {
      int changeLength = plainTextChange.getInserted().length()
        - plainTextChange.getRemoved().length();
      if (changeLength != 0) {
        ...
        if (indexOfChange < caretPosition) {
          ...
          positionCaret(caretPosition < endOfChange ? indexOfChange
        ...
        if (selectionStart != selectionEnd) {
          // if start/end is within the changed content, move it to
          // indexOfChange
          // otherwise, offset it by changeLength
          // Note: if both are moved to indexOfChange, selection is empty.
          if (indexOfChange < selectionStart) {
            selectionStart = selectionStart < endOfChange ? indexOfChange
              : selectionStart + changeLength;
          }
          if (indexOfChange < selectionEnd) {
            selectionEnd = selectionEnd < endOfChange ? indexOfChange
    ...
    Val<Position> caretPosition2D = Val.create(() -> content.offsetToPosition(
      internalCaretPosition.getValue(), Forward), internalCaretPosition,
      paragraphs);
    ...
    internalSelection.addListener(obs -> {
      ...
      selectionEnd2D = sel.getLength() == 0 ? selectionStart2D
        : selectionStart2D.offsetBy(sel.getLength(), Backward);
    });

    selectedText = Val.create(() -> content.getText(internalSelection
      .getValue()), internalSelection, content.getParagraphs()).suspendable();

    final Suspendable omniSuspendable = Suspendable.combine(beingUpdated, // must
                                                                          // be
                                                                          // first,
                                                                          // to
                                                                          // be
                                                                          ...
                                                                          // to
                                                                          // release
      text, length, caretPosition, anchor, selection, selectedText,
      currentParagraph, caretColumn,

      // add streams after properties, to be released before them
      plainTextChanges, richTextChanges,

      // paragraphs to be released first
      paragraphs);
    manageSubscription(omniSuspendable.suspendWhen(content
  ...
  public IndexRange getParagraphSelection(int paragraph) {
    ...
    int end = paragraph == endPar ? selectionEnd2D.getMinor()
      : paragraphs.get(paragraph).length();
  ...
  public void replaceText(int start, int end, String text) {
    StyledDocument<PS, S> doc = ReadOnlyStyledDocument.fromString(text,
      getParagraphStyleForInsertionAt(start), getStyleForInsertionAt(start));
  ...
  public void selectRange(int anchor, int caretPosition) {
    try (Guard g = suspend(this.caretPosition, currentParagraph, caretColumn,
      this.anchor, selection, selectedText)) {
      this.internalCaretPosition.setValue(clamp(0, caretPosition, getLength()));
      this.anchor.setValue(clamp(0, anchor, getLength()));
      this.internalSelection.setValue(IndexRange.normalize(getAnchor(),
        getCaretPosition()));
    }
  }
  ...
  private UndoManager createPlainUndoManager(UndoManagerFactory factory) {
    Consumer<PlainTextChange> apply = change -> replaceText(change
      .getPosition(), change.getPosition() + change.getRemoved().length(),
      change.getInserted());
    BiFunction<PlainTextChange, PlainTextChange, Optional<PlainTextChange>> merge =
      PlainTextChange::mergeWith;
    return factory.create(plainTextChanges(), PlainTextChange::invert, apply,
      merge);
  }

  private UndoManager createRichUndoManager(UndoManagerFactory factory) {
    Consumer<RichTextChange<PS, S>> apply = change -> replace(change
      .getPosition(), change.getPosition() + change.getRemoved().length(),
      change.getInserted());
    BiFunction<RichTextChange<PS, S>, RichTextChange<PS, S>, Optional<RichTextChange<PS, S>>> merge =
      RichTextChange<PS, S>::mergeWith;
    return factory.create(richChanges(), RichTextChange::invert, apply, merge);
  }

  private Guard suspend(Suspendable... suspendables) {
    return Suspendable.combine(beingUpdated, Suspendable.combine(suspendables))
      .suspend();
  }
}
