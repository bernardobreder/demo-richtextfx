...
abstract class StyledDocumentBase<PS, S, L extends List<Paragraph<PS, S>>>
  ...
  protected StyledDocumentBase(L paragraphs) {
    ...
    navigator = new TwoLevelNavigator(paragraphs::size, i -> paragraphs.get(i)
      .length() + (i == paragraphs.size() - 1 ? 0 : 1));
  }
  ...
  public String getText(int start, int end) {
    return sub(start, end, Paragraph::getText, Paragraph::substring,
      pars -> String.join("\n", pars));
  }

  @Override
  public String toString() {
    return paragraphs.stream().map(Paragraph::toString).reduce((p1, p2) -> p1
      + "\n" + p2).orElse("");
  }
  ...
  public StyledDocument<PS, S> subSequence(int start, int end) {
    return sub(start, end, p -> p, Paragraph::subSequence, (
      List<Paragraph<PS, S>> pars) -> new ReadOnlyStyledDocument<>(pars,
        ADOPT));
  }

  @Override
  public StyledDocument<PS, S> subDocument(int paragraphIndex) {
    return new ReadOnlyStyledDocument<>(Arrays.asList(paragraphs.get(
      paragraphIndex)), ADOPT);
  }
  ...
  public StyleSpans<S> getStyleSpans(int from, int to) {
    ...
    if (startParIdx == endParIdx) {
      ...
      subSpans.add(startPar.getStyleSpans(start.getMinor(), startPar.length()
        + 1));
   ...
   * @param combine combines mapped paragraphs to form the result. It is safe
   *        for the client code to take ownership of the list instance passed to
   *        combine.
   * @param <P> type to which paragraphs are mapped.
   * @param <R> type of the resulting sub-document.
   */
  private <P, R> R sub(int start, int end, Function<Paragraph<PS, S>, P> map,
    SubMap<Paragraph<PS, S>, P> subMap, Function<List<P>, R> combine) {

    Position start2D = navigator.offsetToPosition(start, Forward);
    Position end2D = end == start ? start2D
